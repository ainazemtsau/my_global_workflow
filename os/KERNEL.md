...
- **G3 (appetite).** Appetite is the size of the shaped work, not its duration — never legs, days or dates: a leg is one job (§1), so `three legs` reads as three chats and three days at once. Set before tasks, it never extends: work beyond it dies and continuation requires a new shape. A clock enters only in the owner's own words; otherwise elapsed time is reported, never a threshold, and never kills a bet.
