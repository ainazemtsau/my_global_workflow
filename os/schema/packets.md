...
Every `kind: engineering` packet carries explicit `packet-stage: intent|finalized`, one normalized `phase: SETUP|PLAN|EXECUTE|BUILD|DELIVER|CLOSE|RE-SYNC`, and `worktree: <absolute target path>`; `repo` remains logical routing only. SETUP also carries `setup-stage: interview|adapter-author|adapter-review|install`; PLAN carries `plan-stage: author|verdict`; EXECUTE/BUILD carry `execution-mode: fresh|resume`. Missing/unknown/stage-incompatible identity is invalid.
...
SETUP bootstrap has legal pre-install stages. `SETUP/interview` is branch/base-free, read-only, requires no adapter or install manifest, and returns a Direction RESULT recording exact target inventory plus selected stack/profile blobs; any product write is invalid. Only after its writer event may `SETUP/adapter-author` return one declared `work/validation-adapters/<stack>-v21.*` artifact from the pinned OS corpus. Writer commits that author RESULT/artifact plus a normal mechanical finalization CALL; a fresh finalizer RESULT creates `SETUP/adapter-review` in a strict descendant commit. That fresh read-only review refutes canonical materialization and returns the accepted exact path/mode/blob/output manifest. Adapter author, reviewer and installer/checker are distinct. `SETUP/install` is later and cites the ancestral interview, author and review events; missing/stale event, missing author artifact or changed blob stops before product writes.
...
- `PLAN/author` intent carries change-id, exact entry diff, installed toolchain identity, final conformance event/receipt and typed TESTABILITY; it never accepts owner verdict. Pinned plan returns PLAN-GATED checkpoint with candidate, actual zero-exit observation and receipt. Checkpoint apply reruns/recomputes and commits Direction event W. The same RESULT creates a normal pending `kind: mechanical` finalization CALL whose stable child intent binds event path/blob but cannot yet name W; it creates no verdict CALL. A fresh session runs that CALL and emits its own RESULT. Only its declared state_changes may resolve W, append history/LOG, remove the finalization CALL and create finalized `PLAN/verdict`; writer applies it as W2. W is a strict ancestor of W2. Collect then runs the verdict CALL; owner words arrive in event W3 with another ordinary finalization CALL. A different fresh session returns the mechanical RESULT that finalizes EXEC in W4. A bare writer edit, self-reference, missing RESULT/history/LOG, double finalization, extra live delta, changed stable child field, owner/product work during finalization, or missing/non-ancestral W/W2/W3/W4 is invalid.
