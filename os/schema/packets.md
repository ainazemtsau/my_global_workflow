...
Every `kind: engineering` CALL also carries exactly one normalized `phase: SETUP | PLAN | EXECUTE | BUILD | RE-SYNC`. Missing or unknown phase is invalid. `SETUP` is legal only while the target lacks a complete installed root run-contract plus readable validation stamp: it performs PROJECT_SETUP's interactive stack interview, initializes or finishes git/contract/toolchain while preserving existing files, and may not implement a product feature. Because repository identity may not exist yet, SETUP carries `worktree: <absolute target path>` but has no branch/base requirement; an initialized target rejects SETUP. `PLAN` is the owner-interactive plan leg and stops at approval; `EXECUTE`/`BUILD` is the separate fresh feature implementation leg; `RE-SYNC` only installs a newer engineering contract into an initialized behind product and may not implement the product feature. PLAN/EXECUTE/BUILD/RE-SYNC against an uninitialized target is invalid. Every `EXECUTE`/`BUILD`/`RE-SYNC` CALL carries exact `worktree: <absolute path>`, `branch: <name>`, and authoritative `base: <sha>`. When EXECUTE/BUILD names a frozen/OpenSpec change, these further top-level fields are mandatory: `change-id: <id>`, `testability: openspec/changes/<same-id>/TESTABILITY.md`, `baseline-commit: <same sha as base>`, `input-manifest-hash: <sha256>`, `complete-sweep: yes`, and `verdict: GREEN`. Missing values, aliases such as `BUILD-only`, a path/id/base mismatch, and prose in `goal`/`context` are invalid; they do not substitute for normalized fields.
