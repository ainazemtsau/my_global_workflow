...
Every `kind: engineering` CALL also carries exactly one normalized `phase: PLAN | EXECUTE | BUILD | RE-SYNC`. Missing or unknown phase is invalid. `PLAN` is the owner-interactive plan leg and stops at approval; `EXECUTE`/`BUILD` is the separate fresh feature implementation leg; `RE-SYNC` is the only leg allowed to install a newer engineering contract into a behind product and may not implement the product feature. Every `EXECUTE`/`BUILD`/`RE-SYNC` CALL also carries exact `worktree: <absolute path>`, `branch: <name>`, and authoritative `base: <sha>`. When EXECUTE/BUILD names a frozen/OpenSpec change, these further top-level fields are mandatory: `change-id: <id>`, `testability: openspec/changes/<same-id>/TESTABILITY.md`, `baseline-commit: <same sha as base>`, `input-manifest-hash: <sha256>`, `complete-sweep: yes`, and `verdict: GREEN`. Missing values, aliases such as `BUILD-only`, a path/id/base mismatch, and prose in `goal`/`context` are invalid; they do not substitute for normalized fields.
