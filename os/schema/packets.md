...
- `PLAN/author` intent carries change-id, exact entry diff, installed toolchain identity, and `validation-conformance-receipt` plus its writer-owned event; it never accepts owner verdict. Pinned plan returns a terminal PLAN-GATED checkpoint with candidate, zero-exit observation and receipt. Checkpoint apply reruns/recomputes, writes Direction history and commits event W, then stops without a verdict CALL (so no commit self-reference). A fresh writer-finalization job reads W, creates the finalized `PLAN/verdict` CALL carrying `plan-receipt-event: history/<path>@W`, and commits that CALL as W2 where W is a strict ancestor; collect runs only W2. The fresh verdict session presents that candidate and returns actual owner words in a later RESULT/commit W3. Receipt first appearing with words, verdict in author stage, missing/non-ancestral W/W2/W3, packet edit, nonzero/tampered observation or candidate/receipt mismatch is invalid. Only then may writer finalize EXEC.
  The W checkpoint persists a stable `packet-stage: intent` verdict CALL. The next mechanical writer command is exactly `finalize <call-id>`: it may add only enumerated writer fields/event reference and commits W2. It is not collect and runs no owner/product session. `collect` remains invalid until W2.
