...
| WIP-лимиты работают по закону Литтла; переключение контекста съедает до 20-40% ([APA](https://www.apa.org/topics/research/multitasking)) | Гейт G1; `track_wip_limit`; pulse-пункт 9 о распылении владельца |
...
| Еженедельный обзор — петля, без которой система перестаёт быть доверенной ([GTD weekly review](https://gettingthingsdone.com/wp-content/uploads/2014/10/Weekly_Review_Checklist.pdf)); стадийные гейты Go/Kill по заранее заданным критериям ([Cooper, Stage-Gate](https://www.stage-gate.com/blog/the-stage-gate-model-an-overview/)) | pulse (11 булевых пунктов); review как гейт бета |
| Search plane: дефолтная LLM-сессия выдаёт моду распределения, разные сессии сходятся к одним идеям, self-evaluation слаб; контрмеры — структурные (разные входы / независимость / различимость / внешняя калибровка), не промптовые. Полная доказательная база и адверсариальная проверка — в proposal-доках: SESSION_CRAFT_PROPOSAL.md, strategic_search_mode_direction_os_ru.md, deep-research-report(1).md, CREATIVE_EDGE_PROPOSAL.md | research: strategic_search / nominal-group / miner-briefs; map: human-first + search-first + edge-поле + один редкий стимул + noun-test; shape: Berg equal-footing + инкубация + probe-поля (forecast/against/next_if) + AI-burst evaluator; review: forecast-check + harvest edges; pulse-11 market contact; CHARTER: edges / risk_posture |
