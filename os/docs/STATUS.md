...
updated: 2026-07-22 (bounded outcome dispatch + global selector removal; historical pilot snapshot below remains dated 2026-06-12)
...
- **Track dispatch** (2026-07-17; bounded outcome extension and selector removal 2026-07-22): optional `NOW.tracks` группирует ready/waiting/blocked/paused `open_calls`; owner-approved WIP ограничивает root/decision-треки; `primary` один владеет бетом/tasks; ≤1 ordinary root/track. At most one owner-approved `outcome_dispatch` track may add/expire one auxiliary disposition-only request per target; both endpoint roots stay non-paused and already occupy WIP, while target keeps its plan and implementation authority. `open_calls` + pending `decisions` are the sole dispatch state; `RESULT.next` is a local handoff; several actionable items produce a choice without stored focus. Writer merges stable ids/receipts; old selector residue is repair-only migration data.
