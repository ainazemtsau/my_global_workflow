...
updated: 2026-07-23 (goal-anchored, launch-aware control; historical pilot snapshot below remains dated 2026-06-12)
...
- **Track dispatch** (2026-07-17; bounded outcome extension/selector removal 2026-07-22; control repair 2026-07-23): optional `NOW.tracks` группирует ready/running/waiting/blocked/paused `open_calls`; `running` is a durable owner/runtime launch receipt and is never reoffered. Owner-approved WIP ограничивает root/decision-треки; `primary` один владеет бетом/tasks; ≤1 ordinary root/track. At most one owner-approved `outcome_dispatch` track may add/expire one auxiliary disposition-only request per target; its work-cycle preserves the owner-named objective, ranks direct progress over RESULT recency, and renders one-screen focus/running/0..N launch-now/on-return. Both endpoint roots stay non-paused and already occupy WIP, while target keeps its plan and implementation authority. `open_calls` + pending `decisions` are the sole dispatch state; `RESULT.next` is a local handoff/refill, not a one-work quota. Writer merges stable ids/receipts; old selector residue is repair-only migration data.
