...
- **Потолок бумажной фазы (G3):** у бумажного слоя тот же потолок, что у бета. Первый FAIL проверки возвращает названные строки в converge; ВТОРОЙ FAIL закрывает бумажную фазу узла — третьего круга починки нет, вместо него одно решение владельца (резать, делить в map, или идти в shape с открытыми строками как заглушками), и рекомендация — резать, когда находки бьют по строкам, которые написала предыдущая починка. Третий круг открывают только его слова либо его же переутверждённая карточка (G9). Замерено: g-8f31 и g-12fd по два FAIL и ноль кода, g-37a1 core-rows — четыре круга, PASS и через три дня `obsolete`; во всех трёх наряд третьей починки останавливала рука владельца, а не правило.
