...
> **SUPERSEDED IN PART, 2026-08-01.** Everything below describes the layer as designed on 2026-06-14. Three of its load-bearing pieces were REMOVED after measurement (four `heavy` nodes, zero product lines): recursive mechanism decomposition, the zero-open-rows closure predicate, and the self-authored independent oracle. Read the §Derivability revision at the end of this file and `os/plays/converge*.md` for the live contract; the sections marked SUPERSEDED below are kept as the record of why the layer was built, not as rules.

## Core insight: convergence is recursive (fractal), not one-shot — SUPERSEDED 2026-08-01
...
So convergence must re-fire on each committed mechanism, each time minting that mechanism's own forced questions. This is the load-bearing fix; everything else is machinery around it. **— SUPERSEDED 2026-08-01: this recursion was the row-explosion engine (ten owner criteria → 89 rows on one node) and is deleted; a committed mechanism is ONE row and its parameters are born at PLAN time, with the owner present. The witness defect it was built for (parameters invented at task time) is now held by the `answered`-needs-an-outside-source rule plus the `→ PLAN` route, not by minting the parameters on paper first.** (Two independent design passes — the prior chat's v3.1 and this one — arrived at recursion independently, which is evidence the shape is right.)
...
- **converge** (TIER + DEFINE + RESOLVE) — type the node by risk (ceremony is a ceiling); lock disputed terms mechanically; resolve the WHAT as a cited node-on-paper. Questions derive from TWO sources: every done_when criterion and every cross-node edge (a committed mechanism is ONE row; its parameters are born at PLAN time — the recursive third source was removed 2026-08-01 as the row-explosion engine). HOW magnitudes/formats are firewalled `→ PLAN`; acceptance PROPERTIES and owner-owned design facts are owner-signed WHAT.
...
2. **Mechanism decomposition made mechanical** (converge step 3). Any committed mechanism (chosen approach, or a mechanism locked in done_when) is atomically split into one question per internal parameter — so Q/N/K are *derived*, not dependent on how deeply the architect happened to decompose. **— DELETED 2026-08-01, see the SUPERSEDED note above.**
...
- **REVISED 2026-08-01 (measured).** The layer no longer promises a CLOSED question set. Demanding zero open rows, with the owner absent and technical forks barred from reaching him, forced the leg to invent the answers: four `heavy` nodes produced 89, 27+ and 15 leg-authored load-bearing rows and zero product lines, while verification generated new findings out of each repair. `answered` now requires a source outside the deciding leg, and an open row is legal and travels to shape as its stub-and-cut agenda. What the layer still guarantees: no row is load-bearing without a traceable source, every done_when clause reaches a row, and every hole is NAMED with its answerer. Completeness is settled by building.
...
Built via two multi-agent passes (5 independent architectures → adversarial critique → synthesis; then a 3-reviewer validation of the authored plays incl. a dogfood run on g-9c41). The dogfood caught four retrofit-first-run gaps, now fixed in the plays: (a) converge-verify's independent oracle is authored from first principles on a first run when no knowledge/ checklist exists — an empty oracle is a BLOCKED close, never auto-PASS **[REVERSED 2026-08-01: a checklist the verifying leg writes is a `capture`, never a standard and never a finding — measured, one verify minted ten dimensions and the next extended them to thirteen, so the judge was invented by the judge]**; (b) an imported born-closed row that NAMES a mechanism still runs Resolve 3(c) decomposition (closing a mechanism never closes its parameters) — without this, a retrofit swallows the chunked-delta stream and never mints Q/N/K **[DELETED 2026-08-01 with Resolve 3(c) itself]**; (c) a row carrying both an acceptance property AND a HOW magnitude (the witness "divergence = Q") SPLITS — property → G7 WHAT, magnitude → PLAN — never routed whole; (d) history import is scanned and the imported ids listed in a play_check line so a missed prior decision is visible, not silent.
