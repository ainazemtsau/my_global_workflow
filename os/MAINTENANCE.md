...
- **Mandatory-reading budget.** All tracked text under `os/engineering/**`, including contract
  history and the inventory, is mandatory reading with a ceiling of `32,511` words at
  contract v25. The measurement is the whitespace-token count over the tracked files. Later
  maintenance may not raise the ceiling; B4 lowers it only after proved deletions, never by packing
  duties into longer lines.
