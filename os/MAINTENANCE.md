...
1. **Locate.** Read `os/FRICTION.md` and the named files; grep for the responsible rule. Restate the problem in ≤3 lines and classify:
   ...
   **Due watches (same read, no extra file).** `os/FRICTION.md` carries entries parked on `→ logged, watching [watch:<slug>]` — a condition with no actor until now. While you are in that file, count the keys: `grep "^20" os/FRICTION.md | grep -o "watch:[a-z0-9-]\+" | sort | uniq -c | sort -rn` (entry lines only — the file header carries the word too). A slug on ≥2 entries with no later `→ fixed watch:<slug>` is DUE — its own repeat condition has already been met, and it is exactly the ≥2 threshold of step 2. If a due watch IS this session's case, say so and resolve it here: that is the same problem, not a second one. If it is not, name every due slug in the step-6 reply so the owner can dispatch it — a due watch is never left silent, and it is never converted into a direction card.
...
2. **Trigger check.** An explicit owner request is a sufficient trigger for (a)–(c). Self-initiated improvements (nobody asked) need ≥2 FRICTION entries on the same point. Never both fix the request AND slip in unrelated "improvements". Record the trigger now, verbatim: an owner-triggered change carries his exact words in quotes into the step-6 FRICTION line, so a later reader can check what he actually asked for.
3. **Design the smallest change** that resolves the case, inside the budgets: kernel ≤1800 words, a play ≤600, five state file types, two packet types.
   **Migration clause:** a change that makes existing `live/**` state illegal must name, in the same commit, which surfaces must survive it and which repair route applies. "A separate repair RESULT is required" is not a migration — without the survivor list, repair reads the new rule as a mandate to clear. If the fix wants to exceed a budget — simplify the fix or propose removing something else in the same change; growing past budgets is solving the wrong problem.
