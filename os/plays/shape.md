...
9. **Close** — show one screen: appetite, tasks, cuts, lens verdicts, kill_by, forecast and against. On approval, RESULT makes the node `active`, writes the bet, declares any lanes the owner named in exact words, registers its first-task CALL in `open_calls`, and hands it off through `RESULT.next`.
