...
1. **Triage & import** — classify by RISK, failing safe toward MORE ceremony. First SCAN knowledge/ + history/ for owner-approved decisions: each enters §WHAT `status: answered, source: history/<id>`, born-closed; list the imported ids in the header. An imported answer that NAMES a mechanism still runs Resolve 3(c) — closing a mechanism never closes its parameters. Then type: trivial (no disputed term, no sibling, all criteria safe-to-stub) → converge OFF, straight to shape; standard (≥1 term read two ways, OR consumes/produces from a sibling); heavy (model-bearing — wrong stub fatal or defines the product — cross-cutting, or decomposes into subsystems). Write `triage: <type> — converge <OFF|ON> — because <model? contract?>` into the header + opening RESULT. A mistyped-low node bounces from shape's Definition-of-Ready.

2. **Define** — seed terms MECHANICALLY from done_when words; only terms read 2+ ways qualify. §GLOSSARY rows `Term | meaning-here | load-bearing properties | competing-readings`. TERM-PROPERTY RULE: a §WHAT line cites a term only for a property in its row; a new property re-opens and re-signs it. Run ONE miner `call:research` (name every done_when/lens noun + any second reading on disk). Sign disputed terms (G7 — readings ARE the options).
