...
- An `outcome_dispatch` root controls a cycle. Preserve the owner-named outcome; rank direct progress/unblock above receipt recency; keep administrative close background unless blocking. Its owner view is conversation, not telemetry: say plainly where the release is going, what matters now and why, what is underway, what useful independent work may start (0..N), and what changes the advice. Hide ids, play steps, CALL/RESULT terms, status/forecast labels and empty fields; translate them into consequences and dates. Paste text only when asked, titled `Задача для отдельного чата`, never `next CALL`. A same-track continuation is refill, not quota or wait on one result. Launch/loss words change only matching status. Read relevant surfaces. Keep strategy stable, detail in history, housekeeping batched.
