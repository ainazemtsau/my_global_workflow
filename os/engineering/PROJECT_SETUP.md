...
- `validation.config` — gate thresholds: retry budget, **mutation kill-rate floor (a required key — its absence means G2 is not enabled, VALIDATION.md §Enabled ≠ written)**, escalation rules (machine-readable; the run loop reads it).
...
- **Re-sync (existing repos).** Setup runs once, so when `os/engineering` hardens or adds a gate, an already-set-up repo does NOT get it automatically — its deliver check stays stale. Re-sync is a `docs/FRICTION.md` / maintenance trigger IN the product repo: re-run the affected Done-when items below and wire the newly-required gate (this is the general form of the per-gate "retrofitting is a friction trigger" note). Until re-synced, the repo cannot claim the hardened gate is enabled. The os/engineering maintenance change that hardens a gate names the re-sync as a follow-up; it does not itself edit product repos.
