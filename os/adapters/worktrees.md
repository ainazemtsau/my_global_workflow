...
Parallel sessions inside one direction are legal (G1 caps the direction at one active bet, and the owner's WIP limit caps what runs inside it; the open call cards are the dispatch frontier) -- only their *applies* serialize. With `bet: null`, no non-recurring execution session is lawful; the one untracked owner-present authoring CALL for a parked specification outcome is a planning exception, never a track or executor run.
...
Direction tracks are first-class **execution dispatch**, not git isolation, departments, future objectives or extra bets: when present, track cards group mergeable call cards that all resolve to the current `NOW.bet` node/tasks. Future goals remain parked/shaped as node cards; unrelated work waits in issue cards until its owning play admits it. Product executors still isolate writes in product branches/worktrees under their repo contract. Create a new Direction only when work needs a genuinely separate mission/charter/root authority; category names such as another game, canon, visuals, or marketing do not decide that split by themselves.
...
# ...semantic-rebase the RESULT delta, apply it with osctl card/log/leg close (full RESULT lands in history/), maintain END_OF_FILE trailers...
...
3. Track routing lives in track cards (`NOW.md` keeps only the bet pointer and `track_wip_limit`); engineering write isolation lives in the product repo (branches/PRs/cloud VMs per CONTOUR), not in workflow worktrees.
