RESULT s-solmax-operating-substrate-kernel-service-zone-best-practice-research-001 (call: c-solmax-operating-substrate-kernel-service-zone-best-practice-research-001)
direction: solmax
play: research
node/task: g-operating-substrate/c-solmax-operating-substrate-kernel-service-zone-best-practice-research-001
parent: s-solmax-operating-substrate-core-invariant-architecture-forge-001

outcome: |
  Research status: COMPLETE.
  Architecture status: RESEARCH INPUT ONLY — no Q2 card or service-zone
  classification has been approved by this session.
  gap_event: none.
...
evidence: |
  ...
  1. Compared current primary-source systems/guides:
     PASS — eight compared:
     OpenAI Agents SDK, Anthropic, Google ADK, LangGraph, MCP,
     Microsoft AutoGen, Temporal and A2A.

  2. Convergent responsibilities and meaningful disagreements:
     PASS — sections 4 and 5.

  3. Five responsibility layers distinguished:
     PASS — section 6 and classification stress test in section 9.

  4. Portable continuation, chat/run and writer/apply examined:
     PASS — sections 8.1, 8.2 and 8.4.

  5. Failure modes and premature-freeze risks recorded:
     PASS — section 10.

  6. Candidate Q2 criteria derived:
     PASS — section 7.

  7. Hidden prerequisite/gap_event identified:
     PASS — gap_event none; provisional entity vocabulary is inline and
     exact entity model remains an existing downstream child question.

  8. Q1 preserved and no implementation selected:
     PASS — section 1; no provider, framework, carrier, repository, backend,
     storage or application architecture selected.

  9. Ready Q2 architecture-forge CALL:
     PASS — included in RESULT.next.
  ...
  1. OpenAI Agents SDK
     ...
     Supports:
     bounded agent loop, handoffs, sessions, serialized interrupted state,
     approval/resume and tracing.
  ...
  2. Anthropic
     ...
     Supports:
     workflow-vs-agent distinction, composable patterns, bounded
     orchestrator-worker delegation, external checkpoint memory,
     observability, evaluation and version coexistence.
  ...
  3. Google Agent Development Kit
     ...
     Supports:
     session/state/event/memory/artifact separation, event-mediated durable
     mutation, resumable workflow steps, action confirmation, at-least-once
     effect caution and tracing.
  ...
  4. LangGraph
     ...
     Supports:
     thread/checkpoint distinction, interrupts, pending writes, replay/fork,
     idempotent effect boundary and isolated child state.

  5. Model Context Protocol, version 2025-11-25
     ...
     Supports:
     host authority, trust isolation, capability/version negotiation,
     least privilege, structured tool declarations, consent, task lifecycle,
     cancellation, timeout and audit.

  6. Microsoft AutoGen stable documentation
     ...
     Supports:
     serializable agent/team state, handoff teams, bounded termination,
     internal events vs chat messages and backend-neutral tracing.

  7. Temporal Platform documentation
     ...
     Supports:
     durable work identity vs run identity, event history, validated
     messages, isolated external effects, idempotent retry, child work and
     version-compatible long-running execution.
  ...
  8. Agent2Agent Protocol
     ...
     Supports:
     distinction among context, task, message and artifact; task lifecycle;
     input-required continuation; capability/security declaration; polling,
     streaming and push-based asynchronous handoff.

  ## Source limits

  - Framework documentation describes supported mechanisms, not proof that
    each mechanism belongs in every universal process.
  - MCP and A2A specify interoperability boundaries, not complete internal
    process kernels.
  - Temporal supplies strong durability/effect analogies, but deterministic
    workflow replay must not be imposed on arbitrary model reasoning.
  - No source supplies a complete provider-neutral semantic continuation
    export.
  - External sources support human/host authority more strongly than a
    concrete owner-profile object.
  - Direction OS and Zaratusta remain evidence, not authority.
...
state_changes: |
  ...
  - any product or implementation repository

  Mechanical writer should apply exactly:

  1. Create research work artifact:
  ...
  2. Create ready architecture-forge CALL:
  ...
  3. Update live/solmax/NOW.md:
     ...
     - Replace NOW.next with:
       work/calls/c-solmax-operating-substrate-service-zone-classification-architecture-forge-001.md
  ...
  4. Append live/solmax/LOG.md:
     ...
     2026-07-10 — solmax/g-operating-substrate research: Q2 kernel
     service-zone/classification primary-source note completed across OpenAI,
     Anthropic, ADK, LangGraph, MCP, AutoGen, Temporal and A2A; semantic
     responsibility boundaries and candidate five-layer promotion criteria
     derived; no gap_event and no implementation selected; next Q2
     architecture-forge. →
     history/2026-07-10-s-solmax-operating-substrate-kernel-service-zone-best-practice-research-001.md
...
captures:
  - Existing Q1 child question "kernel entity model" should later settle the exact chat/conversation/run/task/procedure relationships; Q2 needs only the inline working vocabulary.
  - Q13 proof should include a cross-carrier continuation scenario that starts a fresh model/chat from the semantic index without access to the old transcript.
  - Q10/Q13 proof should attempt unauthorized, stale and duplicate durable mutations and require explicit rejection or idempotent apply evidence.
  - Q12 should test research, executor and verifier as process-pack/reusable delegation roles rather than universal physical agent classes.
  - External evidence for a first-class owner-profile object is weaker than evidence for host/user authority; its exact schema requires downstream owner-profile work.
  - Observability proof must include privacy/redaction and owner-facing live usefulness, not only complete internal traces.

decisions_needed: []

play_check:
  - 1 Recite: done — exact bounded question, requested RESULT shape, done_when and one-focused-session budget were restated.
  - 2 Investigate: done — all named Solmax/Direction OS inputs were read through END_OF_FILE and eight current primary-source approaches were compared.
  - 3 Confidence: done — established cross-system convergence was separated from candidate Q2 inference; disagreements, source limits, confidence levels and answer-change conditions were recorded.
  - 4 Close: done — research artifact, source register, convergent findings, disagreements, five-layer classification criteria, continuation and writer/apply analysis, failure modes, gap verdict, exact writer state_changes and ready Q2 CALL are returned.

log: |
  solmax research c-solmax-operating-substrate-kernel-service-zone-best-practice-research-001:
  eight-system primary-source Q2 note complete; responsibility boundaries and
  promotion criteria derived; gap_event none; return to parent with ready
  service-zone/classification architecture-forge CALL.

next: |
  return-to-parent s-solmax-operating-substrate-core-invariant-architecture-forge-001
  ...
  goal: |
    One owner-approved Q2 architecture card exists for the operating-substrate
    service-zone and responsibility-classification rule: how a candidate
    concern is classified as invariant kernel semantics, reusable service
    semantics, a process-pack declaration, an owner-profile concern,
    an adapter/implementation concern, or deferred—without freezing a final
    service-block list or selecting an implementation.
  context: |
    Read:
    - live/solmax/CHARTER.md
    - live/solmax/TREE.md
    ...
    - every close/checkpoint yields a portable continuation index;
    - durable mutation crosses a validated writer/apply boundary;
    - exact message schemas, writer topology and final service-block list
      remain open;
    - Markdown/chat are the bootstrap carrier only;
    - Q1 is accepted and must not be revised by Q2.

    Q2 forge handoff:
    - plain_question: "Какие service zones могут быть reusable structure, и
      по какому правилу candidate responsibility becomes substrate-level?"
    - why_now: "Q1 is accepted and the required primary-source research now
      supports a bounded responsibility-classification decision."
    - must_decide: "Candidate definition of service zone; promotion,
      classification and defer criteria; evidence threshold across invariant
      kernel semantics, reusable service semantics, process-pack declarations,
      owner-profile concerns and adapters."
    - expected_answer_shape: taxonomy
    - return_to_graph_if: "The rule cannot consistently classify at least
      persistence, live interface, portable continuation, writer/apply and
      domain-specific safety policy without choosing implementation or
      freezing the block list."

    Research orientation:
    - Current sources converge on semantic boundaries, not a canonical
      physical decomposition.
    - Service zone is a candidate responsibility family, not automatically a
      module, service, process, agent or file.
    - Runtime-local run/thread/task IDs are not sufficient portable
      continuation.
    - The writer/apply invariant is logical validation and authority, not one
      physical writer topology.
    - The research note's candidate responsibility set is an evaluation set,
      not an accepted block list.
    - gap_event from research: none.
  boundaries: |
    Do not revise or reopen the accepted Q1 card.
    Do not freeze a final service-block list.
    Do not select or design message schemas, writer topology, entity schema,
    API, framework, provider, model, repository, backend, carrier, storage
    engine, transport, deployment or application architecture.
    Do not implement.
    Do not change Direction OS.
    Do not promote one consumer's policy, state layout or live interface into
    substrate architecture.
    Do not treat current framework class names or runtime identifiers as
    substrate entities.
  done_when: |
    One atomic Q2 architecture card is explicitly owner-approved and:

    1. defines service zone without equating it to a deployable
       module/service/agent;
    2. defines candidate, promote, classify, defer and reject criteria;
    3. distinguishes invariant kernel semantics, reusable service semantics,
       process-pack declarations, owner-profile concerns and
       adapter/implementation concerns;
    4. states an evidence threshold and implementation-neutral conformance
       pressure;
    5. explicitly classifies portable-continuation responsibility while
       leaving exact schema open;
    6. explicitly classifies logical writer/apply validation while leaving
       physical topology open;
    7. can classify persistence, live interface and domain-specific safety
       policy without contradiction;
    8. records rejected alternatives, child/downstream questions and any
       gap_event;
    9. preserves Q1 and selects no implementation or final block list.
  return: |
    RESULT with the accepted Q2 architecture card, owner approval words,
    rejected alternatives, child/downstream questions, graph/gap verdict,
    exact state_changes and the next graph route.
  budget: one focused session
  parent: s-solmax-operating-substrate-core-invariant-architecture-forge-001
  surface: chatgpt

  END_OF_FILE: live/solmax/work/calls/c-solmax-operating-substrate-service-zone-classification-architecture-forge-001.md

END_OF_FILE: live/solmax/history/2026-07-10-s-solmax-operating-substrate-kernel-service-zone-best-practice-research-001.md