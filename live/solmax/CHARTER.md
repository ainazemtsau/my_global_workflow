...
2. **Другой человек ставит Zaratustra и обновляет её без нас**
   - Человек, не участвовавший в разработке, по публичной инструкции движка
     ставит Zaratustra в пустую папку и доводит её до первого плана дня; затем
     переводит её на новую версию командой обновления — миграции проходят, он
     читает «что нового».
...
- **Строго индивидуальное использование.** Один владелец, одно рабочее место.
  ...
  управленца.»
- **Один публичный продукт.** Мы строим ровно один репозиторий: публичный движок
  Zaratustra — Python-пакет `zaratustra`, команда `zara`, MCP-сервер, схемы,
  шаблоны экспертов, плагины, миграции, документация для ассистентов, образцовое
  рабочее место — без единого личного факта. Место, где человек держит своё
  содержимое, он заводит себе сам; направление его не проектирует, не раздаёт и
  не контролирует, и отдельного публичного шаблона рабочего места не существует.
  Код движка в этом месте не лежит, стоит зависимостью с закреплённой версией.
  Открытый исходник движка допустим; личные данные остаются там, где человек
  работает, и в публичный репозиторий не попадают. Слова владельца 2026-09-04:
  «плагины, всё это должно быть публично»; «у нас не должно быть понимания
  какой-то приватный репозиторий… не должно быть ничего особого в этом приватном
  репозитории»; своим же продуктом он пользуется «как вообще любой третий
  пользователь».
...
- **Движок Zaratustra** (публичный, заводится в волне 0): Python-пакет
  ...
  оно не выдумывается.
- Место, где владелец держит своё содержимое, репозиторием направления НЕ
  является: он заводит его себе сам, как любой другой пользователь (его слово
  2026-09-04). Отдельного публичного шаблона рабочего места нет — раскладку
  разворачивает установленная версия движка.
...
**Mitigation:** the effect-tier gate (tier-2 → owner approval); local-first; the auditable ledger; least-privilege; связь с workflow строго read-only и проверяется, писать нельзя; личные данные живут только там, где человек работает, и в публичный репозиторий не попадают.
