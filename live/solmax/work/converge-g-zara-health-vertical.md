# Converge WHAT — g-zara-health-vertical
...
imported:
  ...
  - S2: live/solmax/cards/g-zara-health-vertical.md
  - S3: live/solmax/history/2026-08-13-s-solmax-zaratustra-map-finalize-026.md
  - S4: live/solmax/history/2026-08-12-s-solmax-zaratustra-map-evidence-025.md
...
triage: standard — converge ON — because owner-visible Area creation, shared
  ...
  boundaries can each be read multiple ways. This artifact closes product
  semantics and acceptance properties only. It does not choose a data/event
  topology, repository, database, framework or execution architecture, so no
  architecture-on-paper is born here.

miner:
  scope: Area; Capability; Operation; Workflow; Run; creation/onboarding;
    Health minimum; nutrition; training; chat; log; report; useful run;
    real run; low-consequence; shared context; Area context; cross-Area read;
    sensitive data; access grant; web request; writer; effect; trace;
    localhost projection; executor path; eval; non-clinical stop.
  result: second readings are made explicit in G1-G13. Owner-owned content is
    answered from S11 or remains one named open row. Cheap mechanisms are
    routed to PLAN without being silently decided.

strategic_search:
  generated: nine placements for shared Area creation, general chat/context
    and web-to-writer behavior across the six-node roadmap.
  survivors:
...
- S1 — Charter: Zaratustra succeeds through recurring reliance in three Areas,
  extensibility without repeated core redesign and one hard process clearly
  better than a plain chat; it remains personal-first, privacy/trust-sensitive
  and read-only toward Direction OS.
- S2 — Live node: node 1 requires explicit typed contracts, a new Health Area,
  qualified execution, evals, localhost projections, three real runs with one
  explicitly useful, and fresh refutation.
- S3 — Map finalization: Health-first, Area -> Capabilities/typed Operations +
  Workflows -> Runs, opaque prose, typed semantic verdicts, registered
  handlers, localhost UI, first-class access paths, old Health as evidence and
  the accepted non-clinical/data boundary.
- S4 — Map evidence: the selected order is useful vertical -> trusted context
  -> qualification -> second-Area extensibility -> recurring daily use ->
  governed improvement; node 1 is a bounded probe, not proof of the whole
  exocortex.
- S5 — Approved map draft: Area is a logical domain, not a physical topology;
  workflows have registered handlers rather than a custom language; UI reads
  typed projections and writes use typed authority/effect contracts.
- S6 — Live-proof boundary: the first external evidence must be one genuine,
  low-consequence, owner-useful end-to-end run; generalized machinery earns
  expansion from observed trace and failure demand.
- S7 — Failure/route evidence: previous work accumulated a large substrate and
  mechanical proof without a useful owner-facing closure; Zaratustra must be
  the real consumer and evidence source rather than another abstract core.
- S8 — Working Health evidence: the owner-used pack supports ordinary chat,
  concrete training and nutrition planning/adaptation, current-day support,
  explicit logs/reports, review/recovery, experiments, continuation and
  non-clinical safety. Its content is requirements evidence, not runtime/state
  authority or a package to copy blindly.
- S9 — Concept Lab evidence: ChatGPT Web creates an immutable non-canonical
  save request containing the exact owner turn; a local locked importer checks
  its typed fields, imports it once, preserves provenance, writes a receipt and
  applies no verdict. Tests cover exact capture, one-time import and pending
  requests on resume.
- S10 — Process Creator evidence: ordinary intent can drive bounded creation,
  validation, activation, live proof and continuation. Its heavy package and
  lifecycle are evidence only. The accepted Zaratustra contour keeps the
  conversational/bounded/validated-apply properties and rejects a renewed
  universal factory, manifest or process language.
- S11 — Owner decisions in this converge conversation:
  - O1: "В начале sales должен полностью все связанное с питанием и с тренировками."
  - O2: "вот возьми, что там есть. Это самый-самый минимум, который хоть что-то был бы полезен."
  - O3: "его не надо вслепую, естественно, переносить, адаптировать"
  - O4: "такой функционал, который может быть применен к любому, ну, вообще какой-то функционал, он, естественно, должен быть вытащен."
  - O5: "он как-то по шаблону создает, ну если где-то надо, кастомизирует, где-то нет. Ну, то есть, в пределах шаблона. То есть он не должен сделать то, что мы вообще не ожидаем."
  - O6: "если я потом, вот мы сделали health, все, окей, работает, я могу потом легко создать любую другую area, которую захочу."
  - O7: "Да, смотри еще, я в принципе согласен"
  - O8: "сразу нужно запланировать какое-то, ну, условное хранилище, так чтобы разные area могли к нему обращаться."
...
|---|---|---|---|---|
| G1 | Area | A long-lived logical domain such as Health that owns its domain vocabulary, policies, state references, Capabilities, Workflows and projections. | [logical-boundary] scopes meaning/authority, not physical storage; [domain-owned] owns its domain semantics; [extensible] another Area can later register without redefining central semantics. | Folder; chat; repository; database; arbitrary topic label. |
| G2 | Capability | An owner-visible ability inside an Area, realized by typed Operations and usable by one or more Workflows. | [owner-visible] named by what it helps the owner do; [domain-semantic] meaning belongs to its Area; [operation-backed] actions are typed, not prompt folklore. | Single prompt; tool permission; whole Area; Workflow. |
...
acceptance: central shared components/handlers may depend on fixed typed
  ...
  interpretation. →GLOSSARY:G1[extensible] →GLOSSARY:G11[no-semantic-parser]

W6 — Is the primary owner interaction still ordinary chat?
status: answered
answer: Yes. The owner may discuss Health or request/create an Area in free
  language. Models interpret that language into separately typed proposed
  actions; deterministic code validates those actions and never derives them
  from wording. →GLOSSARY:G3[prose-opaque] →GLOSSARY:G11[typed-verdict]
  →S5 →S8 →S11/O4
acceptance: no command phrase, form or lexical intent parser is required for
  ordinary owner interaction; any machine action is explicit in a typed object.
  →GLOSSARY:G11[no-semantic-parser]

W7 — What physical product/storage topology implements the shared spine?
status: open
answerer: PLAN
rewrites: repository boundaries, database/files, service/process layout,
  transaction mechanism and deployment topology.
constraint: whatever is chosen must preserve G1[logical-boundary], G7-G9 and
  G10[validated-apply]; one physical store is allowed but cannot erase logical
  ownership/isolation. →S5 →S11/O8

W8 — How is old Health used?
status: answered
answer: The working Health pack at S8 is requirements, scenario and failure
  evidence. Its runtime, lifecycle, directories and state receive no authority;
  owner-visible behavior is explicitly re-authored and may be improved rather
  than copied one-for-one. →S3 →S8 →S11/O2-O3
acceptance: every retained Health behavior names a new typed contract/handler
  and every omitted S8 behavior is explicitly deferred or rejected; wrapping or
  importing the old pack as the new runtime does not satisfy this row.

W9 — What domain breadth must the first Health Area cover?
status: answered
answer: Health covers both Nutrition and Training as owner-visible
  Capabilities from the start; neither is replaced by a generic evidence or
  administrative workflow. →GLOSSARY:G2[owner-visible] →GLOSSARY:G2[domain-semantic]
  →S8 →S11/O1-O2
acceptance: the Health projection and accepted Area definition visibly include
  both Nutrition and Training, backed by typed Operations rather than labels.
  →GLOSSARY:G2[operation-backed]

W10 — What Training behavior is the minimum evidence surface?
status: answered
answer: The surface can discuss and adapt a concrete programme, session or
  week; represent exercises/categories, sets/reps/load when justified; decide
  progress/hold/reduce/substitute; integrate cycling/VR/daily activity,
  recovery and schedule; and adjust the current session from explicit owner
  observations. These are non-clinical general-wellness behaviors. →GLOSSARY:G13[allowed]
  →S8 →S11/O1-O3
acceptance: tests and owner-facing examples cover initial planning, a normal
  update, an adverse/insufficient-context update and return after a break
  without punishment/debt semantics. →GLOSSARY:G13[allowed]

W11 — What Nutrition behavior is the minimum evidence surface?
status: answered
answer: The surface can discuss and adapt a practical menu window with meals,
  recipes, portions, substitutions and shopping; show explicit energy/protein
  orientation only when justified by allowed context; support sweets/social
  flexibility, daily correction and end-of-window review. It remains general
  wellness and never clinical treatment. →GLOSSARY:G13[allowed]
  →S8 →S11/O1-O3
acceptance: tests and owner-facing examples cover plan creation, substitution,
  changed schedule/appetite context, insufficient data and an end-of-window
  review without diagnosis/treatment claims. →GLOSSARY:G13[forbidden]

W12 — What supporting Health behavior must survive adaptation?
status: answered
answer: Ordinary conversation, explicit logs/reports used for decisions,
  saved/not-saved truth, examples/rationale, review decisions, small
  experiments, last-known-good/return after interruption and a portable
  continuation must remain possible. Unrelated conversation is not silently
  promoted to durable Health state. →GLOSSARY:G5[resumable]
  →GLOSSARY:G10[receipt] →S8 →S11/O2-O4
acceptance: a fresh Run can resume from accepted state and identify what was
  saved, what remains opaque conversation and the next legitimate Health
  action without reconstructing the previous transcript. →GLOSSARY:G5[resumable]

W13 — How are the accepted Nutrition/Training behaviors divided into exact
Capabilities, Operations and Workflows?
status: open
answerer: PLAN
rewrites: handler boundaries, Operation granularity, Workflow graph shape and
  version cut.
constraint: the cut must preserve W9-W12 acceptance and cannot use a custom
  language. →GLOSSARY:G2[operation-backed] →GLOSSARY:G4[not-DSL]

W14 — Which exact Health Workflow is the first external proof Run?
status: open
answerer: owner, after PLAN presents one concrete smallest proposal grounded in
...
status: answered
answer: General-wellness, administrative/organizational and self-observation
  work only. →GLOSSARY:G13[allowed] →S3
acceptance: intended use is declared before activation and visible on the
  Workflow/Run; no route may silently widen it. →GLOSSARY:G13[allowed]

W16 — What Health behavior is prohibited and what happens on it?
status: answered
answer: The system must not diagnose, prescribe/recommend treatment, change
  medication or provide time-critical advice. An out-of-scope request stops
  with a plain explicit handoff/block and performs no Health action. →GLOSSARY:G13[forbidden]
  →GLOSSARY:G13[stop-handoff] →S3
acceptance: normal, edge and adversarial cases prove the stop happens before a
  forbidden output/effect; a disclaimer after advice does not pass.

W17 — What sensitive-data boundary applies?
status: answered
answer: Every executor path and Run declares the exact sensitive data it will
  receive, purpose, applicable controls and allowed effects before execution;
  undeclared or unauthorized data fails closed. →GLOSSARY:G13[data-declared]
  →GLOSSARY:G9[purpose-bound] →S3
acceptance: the owner can inspect why each Health/shared datum was included and
  a missing grant prevents delivery to the executor. →GLOSSARY:G9[auditable]

W18 — What logical context layers must exist from node 1?
status: answered
answer: At minimum: Area-owned context, separately curated shared owner
  context, and opaque conversation/artifacts. Their identities and authority
  remain distinct even if one physical store later carries all three.
  →GLOSSARY:G7[authoritative-owner] →GLOSSARY:G8[separate]
  →GLOSSARY:G11[traceable] →S11/O8-O11
acceptance: a stored item has exactly one logical owner/classification; UI and
  traces distinguish Area context, shared owner context and opaque artifacts.

W19 — Does Health automatically receive all Health data?
status: answered
answer: No. Health owns its domain context, but each Operation loads only its
  declared required/optional references for the current purpose. →GLOSSARY:G7[on-demand]
  →GLOSSARY:G9[minimum] →S11/O9-O10
acceptance: a Run omitting an unrelated Health reference proves that Area
  ownership is not automatic prompt injection. →GLOSSARY:G7[default-isolated]

W20 — May one Area read another Area's data?
status: answered
answer: Only through an explicit purpose-bound context reference/access grant
  allowed by the source owner/policy; cross-Area readability is possible but
  never implied by physical co-location. →GLOSSARY:G7[default-isolated]
  →GLOSSARY:G9[purpose-bound] →GLOSSARY:G9[deny-by-default] →S11/O8-O10
acceptance: an allowed cross-Area case loads the exact granted datum and a
  parallel ungranted case fails without copying or summarizing it.

W21 — What belongs in shared owner context?
status: answered
answer: Only an explicitly accepted typed fact whose expected usefulness spans
  Areas, with source, scope, currentness, sensitivity and allowed use. No fixed
  exhaustive category list is required; promotion is per fact and reversible/
  reviewable. →GLOSSARY:G8[explicit-promotion] →GLOSSARY:G8[provenance]
  →S11/O11
acceptance: node 1 demonstrates at least one owner-confirmed shared fact used
  by an authorized Operation and not delivered to an unrelated Run.
  →GLOSSARY:G8[not-global-prompt]

W22 — Is the owner's kitchen-equipment example already a stored fact or
mandatory schema field?
status: answered
answer: No. It is evidence of the class of cross-Area facts the system must be
  able to represent; its value becomes state only through explicit owner-
  confirmed capture. →GLOSSARY:G8[explicit-promotion] →S11/O11
acceptance: examples never enter shared context merely because they appeared
  in conversation or this specification. →GLOSSARY:G11[no-semantic-parser]

W23 — Must common and Area data use one physical store?
status: answered
answer: No physical choice is made. One store, several stores or replaceable
  bindings may satisfy the logical boundary only if G7-G9 remain enforceable.
  →GLOSSARY:G1[logical-boundary] →S5 →S11/O8
acceptance: shape may choose a reversible initial binding but cannot turn
  physical co-location into universal read authority.

W24 — What minimum properties must a structured context item carry?
status: answered
answer: Stable identity, logical owner, type, source/provenance, version or
  observed-at time, sensitivity/trust class, currentness/expiry when relevant,
  allowed uses and readable/writable effect boundary. →GLOSSARY:G7[authoritative-owner]
  →GLOSSARY:G8[provenance] →GLOSSARY:G9[purpose-bound] →S2 →S3
acceptance: missing owner/source/sensitivity/access information blocks context
  delivery rather than being inferred from prose.

W25 — Can deterministic code extract shared facts or permissions from chats,
reports or artifacts?
status: answered
answer: No. A person/model may propose a typed fact, reference or semantic
  verdict; deterministic code validates only the typed object and explicit
  links. →GLOSSARY:G3[prose-opaque] →GLOSSARY:G11[no-semantic-parser]
  →GLOSSARY:G11[typed-verdict] →S3 →S5
acceptance: adversarial prose containing apparent approvals, ids or commands
  changes no fact, grant, state or effect without a separate typed action.

W26 — Which storage-security mechanisms and parameter values are chosen now?
status: open
answerer: PLAN
rewrites: exact authentication, encryption, retention, redaction, deletion,
  backup, locking/concurrency, trust labels and size/freshness defaults.
...
status: answered
answer: At least applied/saved, rejected, conflict/stale basis, failed-known-
  not-done, partial and outcome-unknown; partial/unknown blocks blind retry.
  →GLOSSARY:G10[receipt] →S9 →S10
acceptance: every requested target has an observed after-state or explicit
  unresolved disposition; a tool call, generated patch or commit alone is not
  a saved claim.

W30 — Is ordinary conversation automatically persisted?
status: answered
answer: No. The system distinguishes ephemeral conversation, an explicit
  proposed save/change request, and accepted durable state; it reports the
  current state honestly. →GLOSSARY:G10[no-effect-by-request]
  →GLOSSARY:G11[traceable] →S8 →S9
acceptance: a test conversation with no typed save action leaves durable
  Area/shared state unchanged and visibly reports that fact.
...
acceptance: versioned normal and invalid fixtures prove identities/references/
  enums/effect bounds deterministically without semantic prose parsing.

W32 — What must every Workflow Step declare?
status: answered
answer: Typed input/output, required/optional context refs, allowed effects and
  writes, possible next transitions, executor requirements and closure states.
  →GLOSSARY:G3[typed] →GLOSSARY:G9[minimum] →S2
acceptance: missing, ambiguous, stale or unauthorized required input fails
  closed before handler/effect; deterministic code never fills it from prose.
  →GLOSSARY:G3[prose-opaque]

W33 — How are Operations and Workflows executed?
status: answered
answer: Explicit registered handlers/adapters execute Operations and
  transitions. Metadata validates, routes, projects and traces; it is not a
  custom executable language. →GLOSSARY:G3[registered-handler]
  →GLOSSARY:G4[registered] →GLOSSARY:G4[not-DSL] →S3 →S5
acceptance: adding or changing a Workflow cannot introduce a compiler,
  interpreter, general procedure language or deterministic prose semantics.

W34 — What executor/access result must node 1 show?
status: answered
answer: At least one real local, subscription or cloud/API access path runs
  through a registered adapter. The owner sees Area/Capability/Workflow,
  recommended model/provider/access mode, reason, alternatives and exact owner
  action. Other access modes remain first-class representable paths. →S2 →S3
acceptance: the exact executed configuration is traceable; no silent provider,
  account, access-mode or data-boundary substitution is allowed.

W35 — What is the general fail-closed rule?
status: answered
answer: Missing/ambiguous input, context grant, authority, executor capability,
  effect scope or transition returns a typed bounded owner question/diagnostic
  block/recoverable failure rather than guessed semantics. →GLOSSARY:G3[typed]
  →GLOSSARY:G9[deny-by-default] →S2 →S5
acceptance: adversarial tests cover each missing boundary and prove no handler
  or effect ran.

W36 — What Eval Harness is required before expansion?
status: answered
answer: Versioned normal, edge and adversarial cases for the selected Health
  and shared paths; deterministic validation for exact contracts; owner/model
...
status: answered
answer: The owner completes at least three genuine low-consequence Runs; at
  least one is explicitly useful. Every other Run ends in honest success,
  bounded owner question, diagnostic block or recoverable failure. The first
  external proof event is one useful end-to-end Run through the thinnest safe
  path. →GLOSSARY:G5[honest-closure] →GLOSSARY:G12[real]
  →GLOSSARY:G12[owner-useful] →S2 →S6
acceptance: fixtures, simulated traces, activation, UI navigation or model
  self-evaluation do not count as real/useful Runs; the owner's exact judgment
  and trace/closure evidence are required.

W39 — When may generalized machinery expand?
status: answered
answer: After the accepted minimum typed safety/context/effect envelope and
  localhost projection exist, contracts, UI, eval and lifecycle machinery
  expand only from observed Run trace/failure demand or the later node's own
  proof obligation. →S6 →S7
acceptance: shape rejects platform work not required by the selected Health
  path, W15-W38 safety/observability, or an observed failure.

W40 — What independent close is required?
status: answered
answer: A fresh separate converge-verify first refutes this WHAT for coverage,
  smuggling and backward/forward cleanliness. Later node completion still
  requires fresh refutation of contracts, opaque prose, privacy/effects,
  context isolation, web writer truth, evals, projections and real-use claims.
  →S2 →S3
acceptance: in-session miner/strategic-search reports are non-binding and
  cannot satisfy either fresh verification boundary.

W41 — What belongs to g-zara-trusted-context-state after node 1?
status: answered
answer: Node 1 establishes only the minimum logical context objects, isolation,
  explicit grants and one real use needed by Health. Node 2 owns trustworthy
  continuity at scale: source freshness/trust/taint, conflicting versions,
  budget, indirect-injection separation, portable current state and broader
  recovery. →W18-W26 →S3 →S4
acceptance: node 1 cannot claim the node-2 outcome; node 2 may deepen mechanisms
  without changing G7-G9 semantics.

W42 — What belongs to g-zara-model-qualification-routing?
status: answered
answer: Node 1 records and explains one actual qualified executor path; node 3
  proves economical selection among exact configurations, expiries/rechecks
  and fallbacks across multiple real access paths. →W34 →S3 →S4
acceptance: node 1 does not claim a global best model or economical routing
  system from one successful configuration.
...
status: answered
answer: Node 1 provides only the owner surface and handoff needed for Health;
  node 5 proves recurring use across three Areas, cross-surface packet resume,
  duplicate-effect safety and one hard Workflow better than plain chat.
  →W27-W30 →W37-W38 →S3 →S4
acceptance: initial Health chat/web use is not recurring three-Area reliance.

W45 — What belongs to g-zara-governed-improvement?
status: answered
answer: Node 1 records real traces/failures but does not automatically mutate
  active semantics. Node 6 turns such evidence into isolated, benchmarked,
  owner-activated, rollback-capable candidates. →W38-W39 →S3 →S4
acceptance: no Run, model or writer silently edits active Area/Workflow/
  handler/policy semantics from feedback.

W46 — What is the Direction OS boundary?
status: answered
answer: Direction OS is read-only evidence for Zaratustra. Zaratustra may
  return a typed request/handoff but cannot write Direction OS state.
  →GLOSSARY:G10[no-effect-by-request] →S1 →S3
acceptance: every Zaratustra executor/writer scope excludes Direction OS; a
  request aimed there stops/routes to the legitimate OS writer.

W47 — Which root claims remain unproved after node 1?
status: answered
answer: Recurring reliance across three Areas, general extensibility and one
  hard process clearly better than plain chat remain root/later-node claims.
  Node 1 contributes first real usefulness and evidence only. →S1 →S4
acceptance: reporting distinguishes node-1 evidence from root completion.

W48 — Which remaining cheap implementation choices belong to PLAN?
status: open
answerer: PLAN
rewrites: exact framework, database/files, repository/service topology, UI
...
A4 — acceptance — Health visibly implements both Nutrition and Training and
preserves the W10-W12 working behavior surface or explicitly records each
owner-approved defer/reject. →W8-W12

A5 — acceptance — every retained Health behavior is re-authored on new typed
contracts/handlers; the legacy pack receives no runtime/state authority. →W8

A6 — acceptance — intended use is general-wellness/admin/self-observation;
diagnosis, treatment, medication change and time-critical advice stop before
output/effect with an explicit handoff. →W15-W16

A7 — acceptance — every executor path declares sensitive data/purpose/controls;
missing context/authority/grant fails closed. →W17 →W24 →W32 →W35

A8 — acceptance — Area context, shared owner context and opaque artifacts have
distinct logical owners/classes even if physically co-located. →W18 →W23-W25

A9 — acceptance — an Operation loads only declared context references; same-
Area and cross-Area access are on-demand/purpose-bound/deny-by-default. →W19-W20

A10 — acceptance — at least one owner-confirmed shared fact is used by an
...
A13 — acceptance — one actual registered executor/access path is visible and
...
A18 — acceptance — Zaratustra cannot write Direction OS and cannot silently
self-mutate active Area/Workflow/handler/policy semantics. →W45-W46

A19 — acceptance — node-1 reporting does not claim trusted-context scale,
economical routing, second-Area extensibility, recurring three-Area use,
governed improvement or root completion. →W41-W47
