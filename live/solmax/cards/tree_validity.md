...
note: 'Дерево перевыпущено картой 2026-09-04 и одобрено словом владельца «да по
  ...
  РАСХОЖДЕНИЕ С УСТАВОМ ЗАКРЫТО в тот же день ногой frame по его прямому указанию

  «почини устав»: критерий успеха 2 больше не держится на шаблоне, ограничение

  «Два репозитория» стало «Один публичный продукт», пункт шаблона в Canonical

  repos заменён строкой о том, что место владельца репозиторием направления не

  является, и смягчение в пре-мортеме 6 перевязано. Суть критерия 2 не менялась.


  Полная запись: history/2026-09-04-s-solmax-zaratustra-v2-map-055.md и
