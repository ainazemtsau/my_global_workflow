...
- как минимум четыре игрока на разных машинах могут начать прохождение и дойти до понятного результата; цель — восемь;
...
- Концепт игры выбран и является якорем направления: команда налётчиков совместно выносит добычу из дома через ходы, которые она сама пробивает и закрывает, а хозяин отвечает на шум и следы. Точные механики этим не выбираются — чем переносят груз, где и чем можно пробить проход, кто населяет дом и чем заканчивается заход, решается пробами. Канон пересобирается под этот концепт.
...
- Неснимаемый минимум игры: команда совместно выносит добычу из дома, а путь наружу делает себе сама. Проход у́же груза, и на каждом таком месте у команды выбор — изменить положение груза или расширить проход, обе цены видимы. Сокращение объёма режет всё остальное раньше; если резать больше нечего — переносится дата, а не ядро.
- Кооператив на 4–8 игроков — жёсткое требование, а не пожелание: минимум четыре, цель восемь. Механика, работающая только для одного-двух, ядром этой игры не является. Это требование к форме ядра, и ограничение простоты его не отменяет: простота режет содержание — механики, системы, контент, — но не число игроков.
- Качество концентрируется, а не размазывается: перенос добычи и проходы доводятся до отличного состояния — на них разрешено тратить время, рисковать и переделывать. Всё остальное имеет право быть грубым и недоделанным; симуляции сред входят по одной, каждая отдельным решением, общий фреймворк под них не строится. Простота ограничивает количество систем, а не качество ядра.
- Правила и технические ограничения, записанные под прошлый концепт, не наследуются по умолчанию: перед первой ногой нового концепта каждое такое правило либо подтверждается явно, либо снимается. Существующий код по-прежнему не удаляется, но правило, написанное под мёртвый концепт, кодом не является.
...
### 11. Кооператив на восемь развалил всё остальное

Причина: единственное жёсткое требование — 4–8 игроков вокруг одного физического груза — оказалось самой дорогой частью и съело время.

Мера: сетевое поведение общего груза проверяется на самой ранней играемой пробе, а не после того, как построены механики.

Граница: если к моменту первой пробы совместное владение грузом не работает у четверых — сокращается механика переноски, а не число игроков.
