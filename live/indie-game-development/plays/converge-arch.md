...
Purpose: close the cross-node CONTRACTS a converged node consumes or produces, and — heavy node only — work its high-risk architecture as a refuted chain into an architecture-on-paper that rides PLAN as input evidence, never into done_when. No gate touched; closure refuted by local/converge-verify before shape.
...
Precondition: converge WHAT spec closed (local/converge done) AND node is sibling-bearing (→ DECLARE) or heavy (→ DECLARE + DECOMPOSE + ARCHITECT). Reached via CALL `to: session, play: local/converge-arch`.
...
2. **Decompose** (heavy) — sketch the node into sub-systems; decide which become sub-nodes (each re-enters TRIAGE via local/converge). Internal seams → internal contracts in §CONTRACTS (WHAT side; HOW stays `→ PLAN`).
...
END_OF_FILE: live/indie-game-development/plays/converge-arch.md
