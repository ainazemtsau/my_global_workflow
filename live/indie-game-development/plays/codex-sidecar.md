...
Precondition: owner names or accepts a mode and target. If target/worktree is unclear or overlaps active Cloud Code work, downgrade to Scout or stop with a decision.

## Steps

1. **Mode + target** — classify the session as `Scout`, `Pair-Lab`, or `Formal Leg`; name target repo/worktree, target scene/files, and what is explicitly forbidden.
