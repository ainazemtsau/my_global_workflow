...
Reads: canon repo (CONSTITUTION + CORE + INDEX + верх QUEUE + SESSION + карточки из поля «нужно»).
...
## Правила (несущие)
1. Одна сессия = один вопрос по умолчанию (кусок ядра из QUEUE.md); кластер — только с названной причиной (проясняются вместе), не произвольно. Следующий кусок/кластер — новая сессия; смежное без причины → в улов. Вход с любого нового чата: «что дальше» само поднимает текущий кусок (START.md).
