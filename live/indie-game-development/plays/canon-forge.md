...
5. **Illustrate (optional)** — only if an image is meaningful: write an image-brief, derive the prompt from `StyleBible.md` + reference plates, generate a SET in ChatGPT (GPT Image), owner picks; commit chosen PNGs under `/assets` (lowercase-hyphenated, relative refs) and reference them in the card. Skip cleanly if no image fits. BLOCKED if `StyleBible.md` is not yet frozen.
