RESULT s-research-launch-control-demo-program-v0-correction-003 (call: owner-delegated-demo-program-v0-stage3-correction-2026-07-23)
direction: indie-game-development   track: launch-control   play: research   node/task: g-b847/demo-program-v0-stage-3-correction
outcome: |
  ...
  - Временной рубеж: [PROVED/STATED] внешний hard gate — demo publicly live before 2026-10-19 10:00 PDT (UTC-7); если выбран October-route, public-readiness receipt должен существовать до начала события.
  ...
  - Временной рубеж: [PROVED/STATED] внешний hard gate — demo live before 2026-10-19 10:00 PDT (UTC-7); отдельный внутренний operational target — Release Demo + base-page republish + anonymous Download Demo smoke complete by 2026-10-19 09:30 PDT (UTC-7); duration/rollback lead UNKNOWN.
  - Proposal: [INFERRED] PROPOSED MUST.
...
evidence: |
  [ESTABLISHED Git] Перед correction-leg tracked worktree был чист; после свежего `git fetch origin main` `HEAD` и `origin/main` оба равнялись `7679dc581ff6dd7f18d45ef7a0eaee52b4c63308`. Исходный кандидат `7430ebe3990a5a5b639588e8e26b7b91379a3355` и binding REFUTED `ca232c1830ac0acf4538c210a6ed153936785e58` являются предками этой базы.
  [ESTABLISHED correction authority] Owner directive требует ровно две temporal-коррекции; binding receipt `live/indie-game-development/history/2026-07-23-s-research-launch-control-review-demo-program-v0-binding-002.md` refuted только date-only границы S0/S14 и предписал тот же внешний pre-10:00 PDT hard gate и отдельный 09:30 PDT operational target.
  [ESTABLISHED exact candidate] Полный `outcome` исходного DRAFT из `live/indie-game-development/history/2026-07-23-s-research-launch-control-demo-program-v0-001.md` воспроизведён без изменений, кроме поля `Временной рубеж` у S0 и S14. Остальные 35 outcome sections и остальные поля S0/S14 сохранены дословно.
  [ESTABLISHED mechanical] Read-only parser над старой и подготовленной новой моделью: `nodes_old=37`, `nodes_new=37`; ровно девять полей у каждого outcome; `changed_fields=[[S0, Временной рубеж], [S14, Временной рубеж]]`; `unchanged_outcome_sections=35`; `unchanged_fields=331`; `non_time_changes=[]`.
  [ESTABLISHED DAG] Dependency edges: `120 → 120`, hash equal; direct-consumer link edges: `120 → 120`, hash equal; `mirror_mismatches_old=[]`, `mirror_mismatches_new=[]`; `acyclic_old=True`, `acyclic_new=True`.
  [LIMIT] Это новый history-only exact DRAFT candidate. Stage 4, принятие/активация Program Model, изменения NOW/open_calls/status/TREE/CHARTER/control-room/product repos, BUILD и binding G5 здесь не выполнялись.
state_changes: |
  1. Add this full RESULT at `live/indie-game-development/history/2026-07-23-s-research-launch-control-demo-program-v0-correction-003.md` with its exact END_OF_FILE trailer.
  ...
  3. Preserve every other file and semantic field unchanged. In particular: no changes to `os/**`, product repos, `CHARTER.md`, `TREE.md`, `NOW.md`/open_calls/status, `work/launch-control/demo-control-room.md`, the original candidate/review history, target tracks, launch-control migration, or any active Program Model; create no track, make no Stage 4/owner verdict, launch no BUILD, and perform no binding G5.
captures: []
decisions_needed: []
play_check:
  - 1 Recite: done — one bounded Stage 3 correction was restated: create one full exact DRAFT candidate by changing only S0/S14 timing, mechanically prove structural identity, and leave all active state/review/build authority untouched.
  - 2 Investigate: done — fresh `origin/main`, clean tracked worktree, exact source candidate, binding REFUTED receipt and required writer rules were read; no subagent, Stage 4, BUILD or binding G5 was used.
  - 3 Confidence: done — an in-memory read-only parser proved 37 outcomes × 9 fields, exactly two changed temporal fields, 35 byte-identical outcome sections, 331 unchanged fields, equal mirrored 120-edge DAGs and acyclicity.
  - 4 Close: done — one complete corrected history-only DRAFT and its exact proof are returned; no model acceptance or review is inferred.
...
END_OF_FILE: live/indie-game-development/history/2026-07-23-s-research-launch-control-demo-program-v0-correction-003.md
