RESULT s-work-near-gas-l1b-resync-v29-dispatch-001 (call: owner-delegated-near-gas-l1b-v29-rescue-2026-07-18)
direction: indie-game-development   track: contract-sync   play: work   node/task: g-9c41/L1B-Capture prerequisite
outcome: |
  Hot-safe NearGas L1B rescue lawfully starts with exactly one bounded GasCoopGame product re-sync CALL marked
  `engineering_contract: re-sync:29`. It installs applicable v28 HOME authority plus terminal v29 hot-safe pin/pair
  authority from the product's current v27 stamp, on the owner-selected WIN-CTRL/Target Local/current dev venue, with
  no feature work and no new v29 feature root before HOME.

  The unmarked cap-reconciliation CALL remains byte-for-byte legacy and runnable only under its recorded contract;
  no legacy successor is issued. A separate owner-approved `contract-sync` parallel track occupies the sixth and final
  WIP slot, while all tasks, decisions, calls, tracks and dirty unrelated bytes remain unchanged.
evidence: |
  owner-authority: «Запускай v29 rescue-route для NearGas L1B. Разрешаю самостоятельно создать и координировать
  Local-задачи в indie-game-development, GasCoopGame_dev и GasCoopGame_win-u3, пересылать CALL/HOME до окончательного
  результата. Сначала hot-safe product re-sync v27→v29 через WIN-CTRL; затем закрыть legacy cap-route без successor и
  открыть новый root engineering_contract: 29. Новый root без изменений принимает frozen PLAN/decision/spec
  c-exec-near-gas-l1b-plan-001; без нового owner interview и без 400-line cap. Качество и независимые gates не снижать».
  direction-authority: `engineering-contract-v27^{commit}` = 64736ecf6071e7ecd26f7e73e87e126b8cbfa0ad;
  `engineering-contract-v28^{commit}` = 9d6e182bad4938a1ec8c23a2dcba5031df8303fc;
  `engineering-contract-v29^{commit}` = 8d2f0866c3e4a9c2f4a41eb60d8fef22584a0a1c; current `CONTRACT_VERSION` = 29.
  v29-authority: current packets/PROJECT_SETUP explicitly permit bounded no-feature Re-sync while pre-v29 legacy calls
  remain open, preserve their artifacts/routes, require new integer pins after HOME, replace the v23 early relay/line
  cap with PAIR-CANDIDATE/fresh PAIR-FREEZE for new v29 roots, and retain every independent quality gate.
  product-readback: WIN-CTRL `C:\projects\Unity\GasCoopGame_dev` is on `dev`, clean, ahead of origin/dev by 7, at
  06b8a6e26cd562fb4650e15552f42794fd2cf291; `validation.config` reports synced/inventory v27, ceiling 239 and source
  64736ecf6071e7ecd26f7e73e87e126b8cbfa0ad. These are dispatch observations, not reset pins.
  artifact: `live/indie-game-development/work/c-exec-near-gas-l1b-resync-v29-001-call.md` is a complete self-contained
  CALL with goal/context/boundaries/done_when/return/budget, exact launch venue, v28/v29 dispositions and HOME contract.
  state-check: current occupancy 5/6 plus the approved contract-sync root = 6/6; ready calls 4→5; the new track has one
  parentless root, primary remains core, and `NOW.next` honestly selects the owner-ordered re-sync prerequisite.
  preservation: existing `c-exec-near-gas-l1b-surface-cap-reconcile-001` entry and artifact are not edited, no L1B task
  closes, and the pre-existing dirty `work/c-work-sphere-universal-capture-frame-001-call.md` is excluded unchanged.
state_changes: |
  live/indie-game-development/NOW.md:
    - SET `updated` to `2026-07-18 by s-work-near-gas-l1b-resync-v29-dispatch-001`.
    - ADD owner-approved track `{id: contract-sync, label: "GasCoopGame — engineering contract re-sync", mode:
      parallel, for: "local: GasCoopGame engineering contract re-sync v29"}`; preserve primary core and every existing
      track. Owner words authorizing Local tasks and ordering re-sync first are quoted in evidence.
    - ADD one parentless root `open_calls[id=c-exec-near-gas-l1b-resync-v29-001]`: track `contract-sync`, status `ready`,
      to `executor`, for `g-9c41 / NearGas L1B v29 rescue prerequisite — bounded GasCoopGame contract re-sync v27→v29`,
      call `work/c-exec-near-gas-l1b-resync-v29-001-call.md`, marker `engineering_contract: re-sync:29`; note pins
      owner-selected WIN-CTRL/GasCoopGame_dev/Target Local/current dev, no feature, exact v28+v29 install, legacy
      preservation and no branch create/switch/reset/stash/clean.
    - SET `next.call` to `c-exec-near-gas-l1b-resync-v29-001` because the owner ordered re-sync before any new root.
    - PRESERVE all tasks, decisions and unrelated open_calls byte-for-byte, including the unmarked legacy
      `c-exec-near-gas-l1b-surface-cap-reconcile-001`; do not create a legacy successor or v29 feature root.
  live/indie-game-development/work/c-exec-near-gas-l1b-resync-v29-001-call.md:
    - CREATE exactly once from the complete CALL in `next` below, with its END_OF_FILE trailer.
  live/indie-game-development/work/board/dashboard.html:
    - REGENERATE the declared live Board/Journal/current NearGas mirrors from fresh NOW+LOG: default is contract-sync
      Re-sync, 9 tracks, WIP 6/6, ready 5 / waiting 0 / blocked 1 / paused 3; explain the new CALL in plain language,
      retain the legacy cap card as pending non-default work, and append this session to 18 July journal.
  live/indie-game-development/LOG.md:
    - APPEND `log` once before its trailer.
  live/indie-game-development/history/2026-07-18-s-work-near-gas-l1b-resync-v29-dispatch-001.md:
    - SAVE this complete RESULT exactly once with its END_OF_FILE trailer.
captures: []
decisions_needed: []
play_check:
  - 1 Recite: done — the job is only the active L1B contract prerequisite: current product v27 becomes v29-ready before any new feature root; no L1B delivery is claimed.
  - 2 Owner inputs (owner): done — this is technical control-plane work; exact owner authority is «Сначала hot-safe product re-sync v27→v29 через WIN-CTRL» and «Качество и независимые gates не снижать», with WIN-CTRL/Local/dev and later-root constraints supplied in the same quoted instruction.
  - 3 Do the work: done — one self-contained `re-sync:29` product CALL is issued on a separate bounded track; no second CALL, legacy successor or product mutation occurs.
  - 4 Self-check: done — exact v27 stamp, v28 HOME delta, v29 pin/pair/no-source-line-cap delta, frozen legacy preservation, WIP 6/6, owner-selected venue and HOME evidence are each dispositioned.
  - 5 Close: done — re-sync is the sole default; cap route remains legacy without artifact rewrite; new v29 L1B root is explicitly deferred until the product HOME and a fresh Direction job.
log: 2026-07-18 · s-work-near-gas-l1b-resync-v29-dispatch-001 · work · contract-sync · g-9c41/L1B-Capture prerequisite: owner-authorized hot-safe GasCoopGame v27→v29 Re-sync is READY on WIN-CTRL/Target Local/current dev as the sole default; WIP is 6/6, legacy cap reconciliation stays unchanged without successor, no feature root or product work opened. → history/2026-07-18-s-work-near-gas-l1b-resync-v29-dispatch-001.md
next: |
...
END_OF_FILE: live/indie-game-development/history/2026-07-18-s-work-near-gas-l1b-resync-v29-dispatch-001.md
