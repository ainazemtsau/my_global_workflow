...
updated: 2026-07-12 (s-repair-near-gas-program-routing-001 — Phase-0 arm dr-20260711-001 закрыт; 6 открытых, остаток finding сохранён)
...
|---|---|---|---|---|---|
| dr-20260711-001 | 2026-07-11 | repair | P1 | **Остаток после gas-routing repair:** Phase-0 arm закрыт `s-repair-near-gas-program-routing-001` — accepted PROGRAM зарегистрирован, старый Phase 0 помечен NOT DELIVERED/NEVER RESUME и удалён из tasks/open_calls/next. Неисправленный остаток finding относится только к P2a0 и старым launch-базам: product-ветка `codex/c-exec-player-puppetmaster-p2a0-002-build@b9aea60c` остаётся CHECKPOINT / NOT DELIVERED без G5/verdict/merge; `knowledge/g9c41-lanes-venues.md:12` называет старый call 001, а база `work/c-shape-sc-damage-call.md:32` остаётся `defade72`. Нужен отдельный узкий repair; gas route повторно не открывать. | known |
