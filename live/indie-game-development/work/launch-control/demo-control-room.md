...
Статус: **ИСПРАВЛЕННЫЙ КАНДИДАТ**. Этот документ ещё не является принятым рабочим планом, новым milestone или разрешением запускать работу. Предыдущий exact blob был опровергнут binding review; эта версия устраняет только три доказанных замечания и снова должна пройти отдельный свежий review, прежде чем владелец сможет исправить или принять её.
...
1. Исправленная точная версия устраняет только три binding findings: внутреннюю дату, глобальный selector и смешанный фокус дня.
...
1. **Изменение мира:** принять Canon replacement workflow как единственный owner-primary; содержание demo и mechanics в этом outcome ещё не выбираются.
2. **Доказательство:** существуют фактические слова владельца, принятый workflow artifact и один законный parented outcome для будущего фактического Demo Experience Tree.
3. **Фон:** новый technical BUILD равен нулю; отдельные Character review/closing и Program release/integration остаются незакрытым product WIP, Grid заблокирован, Gas остаётся отдельным owner-present plan.
4. **Пополнение / пересчёт:** принятие workflow открывает ровно один следующий primary outcome — фактический Demo Experience Tree; binding close Character/Program только пересчитывает capacity и admission Grid, но ничего не запускает автоматически.
...
| Setup, review, owner acceptance и Canon были смешаны в одном дне | Setup прямо назван не-Daily-Brief; первые четыре операционные строки действуют только после принятия и содержат один Canon owner-primary |

### Correction CALL done_when

| Требование correction CALL | Результат самопроверки |
|---|---|
| 1. Один source-linked candidate | **PASS:** статус остаётся исправленным КАНДИДАТОМ; authority/invalidation неизменны; exact blob записывается в RESULT и следующем review CALL |
| 2. External dates отдельно, внутренняя точка честна | **PASS:** hard Steam dates в таблице; 10 августа provisional/inherited; actual latest-safe UNKNOWN до названного evidence |
| 3. Нет глобального selector | **PASS:** в track dispositions нет ярлыков общей очередности; остаток NOW.next не читается и не зеркалируется |
| 4. Setup отделён от Daily Brief | **PASS:** текущая correction/re-review не выдана за world change; первые четыре строки post-acceptance имеют один Canon owner-primary и один refill |
| 5. WIP truth сохранена | **PASS:** Character и Program — разные незакрытые product lineages; limit 1, new BUILD 0, RESULT не освобождает slot; Grid условный candidate, Gas owner-present plan |
| 6. Demo и foreign boundaries сохранены | **PASS:** MUST/SHOULD/CUT, quality, claims, routes, money, min-spec, audio, network, Marketing и девять roots не ослаблены; mechanics/HOW/public action не выбраны |
| 7. Только fresh review до owner acceptance | **PASS FOR ROUTING:** все unknowns остаются в разделе 5; новая точная версия открывает ровно один отдельный refutation review и не заявляет owner verdict |
