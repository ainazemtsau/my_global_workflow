...
Снимок: 22 июля 2026 года, после принятия Canon workflow, регистрации её waiting pilot и свежего опровержения предыдущей версии кандидата.

Статус: **ИСПРАВЛЕННЫЙ КАНДИДАТ · КОРРЕКЦИЯ 2**. Этот документ ещё не является принятым рабочим планом, новым milestone или разрешением запускать работу. Предыдущая версия уже сохраняла три ранних исправления, но была опровергнута ещё по двум точным причинам: она выдавала принятие процесса за изменение продукта/мира и планировала Canon workflow, которая уже принята. Эта версия исправляет только эти два finding и снова должна пройти отдельный свежий review, прежде чем владелец сможет исправить или принять её.
...
|---|---|---|---|---|---|
| 0. Текущая настройка управления | Точная основа Диспетчерской исправляет два current finding, переживает свежую попытку опровержения и получает owner fix-or-accept verdict | Этот кандидат неизменен между новым review и verdict | Одна fresh review session; затем короткое owner decision | Claim-by-claim review RESULT + фактические слова владельца | **IN PROGRESS как setup, не product BUILD** |
| 1. Owner-primary после принятия кандидата | Через существующий Canon pilot владелец принимает, исправляет или отклоняет один непосредственный experience-spine child принятого Demo Contract | Exact acceptance receipt этого кандидата снимает waiting prerequisite; workflow уже принят; pilot выбирается как одна owner-primary session, но не запускается автоматически | Один owner-present Canon slot; не совмещать с Gas decision batch | Pilot RESULT + exact owner words + точная Demo Contract → experience-spine relation | **WAITING на принятие кандидата; затем eligible, не auto-launch** |
...
| Design & Canon | Принятая workflow уже действует; следующий outcome — через существующий pilot принять, исправить или отклонить один непосредственный experience-spine child Demo Contract | Первый owner-primary после принятия Диспетчерской; дальнейшее tree expansion выбирается отдельно | Exact acceptance receipt кандидата, принятый workflow, owner-present pilot | Process receipt может быть ложно выдан за demo progress; случайный технический вопрос может заморозить scope/claims/механику | WAITING на owner-approved Demo Contract; после receipt только eligible, без auto-launch и без design dispatch |
...
- **Owner-primary:** открыть существующий Canon pilot и получить owner approve/correct/reject ровно для одного непосредственного experience-spine child принятого Demo Contract. Workflow уже принята; exact acceptance этого кандидата только снимает waiting prerequisite. Pilot не запускается автоматически, не выбирает mechanics и не создаёт следующий узел сам.
...
|---|---|
| Candidate принят владельцем | Записать exact acceptance receipt; существующий Canon pilot может перейти из WAITING в eligible и стать единственным owner-primary, но не запускается автоматически; перед любым Grid request перечитать NOW |
...
### Текущая setup-последовательность 22 июля — это не Daily Brief

Это служебное завершение Диспетчерской, а не player-visible изменение игры и не операционный фокус уже принятого плана.

1. Три ранних исправления остаются неизменны, а эта точная версия устраняет два current findings: ложную маркировку process progress как product/world result и устаревшую будущую работу по уже принятой Canon workflow.
2. Отдельный fresh reviewer пытается опровергнуть новый exact blob против свежих источников.
3. Verdict could-not-refute открывает один целостный owner fix-or-accept brief; verdict refuted открывает только доказанную correction. До фактических слов владельца операционный Daily Brief не действует и никакой primary outcome не запускается.
...
### Первый операционный Daily Brief — только после принятия кандидата владельцем

Он не привязан к 22 июля и не считается активным без фактических слов владельца. В нём один owner-primary, а не setup плюс условная вторая работа.
...
3. **Чего нет + один owner-primary enabling outcome:** первый experience-spine child и playable loop отсутствуют; после принятия этого кандидата существующий waiting Canon pilot может стать eligible и является единственным owner-primary, но не запускается автоматически и не выбирает mechanics заранее.
4. **Фон + refill/replan:** новый technical BUILD равен нулю; Character review/closing и Program release/integration остаются незакрытым product WIP, Grid заблокирован, Gas остаётся отдельным owner-present plan. После owner verdict pilot Диспетчерская заново показывает result/proof/absence и только затем выбирает refill; ничего не стартует автоматически.
...
| Canon workflow всё ещё планировалась как будущая работа | Runway, Canon row, Active Wave, triggers, working plan и sources используют принятую workflow и текущий pilot WAITING на exact owner acceptance кандидата; eligibility не означает auto-launch |

### Correction-002 CALL done_when

| Требование correction CALL | Результат самопроверки |
|---|---|
| 1. Один unaccepted source-linked candidate | **PASS:** authority/invalidation неизменны; exact новый blob записывается в RESULT и следующем review CALL; принятие владельца не заявлено |
...
| 3. Canon/current-source rebase | **PASS:** workflow уже accepted/published; current pilot WAITING на exact acceptance кандидата; будущая acceptance-step и consumed root удалены; остальные восемь track rows не меняются |
...
| 5. Текущий truthful sample | **PASS:** Canon receipt доказывает только process; experience-spine child, Demo Experience Tree и playable loop отсутствуют; pilot после acceptance только eligible, не auto-launch |
...
| 7. Только fresh review до owner acceptance | **PASS FOR ROUTING:** self-check атакует оба current finding; новая точная версия открывает ровно один отдельный refutation review и не заявляет owner verdict, product launch или foreign dispatch |
