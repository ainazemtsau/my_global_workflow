...
## ЭТА НОГА — ЕДИНСТВЕННЫЙ ВЛАДЕЛЕЦ `IntegratedHouse.unity` НА ВРЕМЯ ВОЛНЫ

Перемер 2026-08-27: `t-reach-wire-1` обязана показать результат в этой же сцене, и если её правят
двое, получается YAML, который не открывал никто. **Файл принадлежит этой ноге**, а провод ждёт её
закрытия. Значит закрываться надо быстро и не расширяться: вернуть сцену в сборку, выяснить запуском
число игроков, довести до четырёх если надо — и всё.

Если по ходу видно, что сцене нужно что-то ещё, — это НЕ делается здесь, а возвращается фактом.
