# NOW snapshot — indie-game-development — 2026-06-29

Reason: pre-compaction snapshot before `s-repair-now-hygiene-001` rewrites
`live/indie-game-development/NOW.md` back to hot state only. This file preserves
the full oversized NOW, including closed calls, answered decisions, and narrative
history, so the canonical entrypoint can stay small without losing evidence.
...
parallel_tracks:   # active ALONGSIDE the g-9c41 bet at owner-set cadence (root map_order); NOT second BETS — no NOW.active_tasks, work via CALLs. G1 intact: one active bet (g-9c41) with its tasks; tracks add none.
  - id: g-7e15
    ...
    appetite: secondary visual cadence in the engine's build-gaps; no fixed hour quota; revisit cadence at pulse. Checkpoint = the P1 prototype gives a min-spec perf signal on the riskiest unknown.
    approach: |
...
END_OF_FILE: live/indie-game-development/work/now-snapshot-2026-06-29.md
