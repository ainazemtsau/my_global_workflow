...
- Read-only dependency audit завершён. Он доказал четыре DELETE-now families: stale Character V1
  builder, Benchmark scene/director/editor, Reaction scene/component и orphaned Dungeon Architect
  GasLab assets со старым guide. Семь scenes остаются временными validators: Sample, Phase0, Buoyancy,
  GasVoxel, GasVisual, analytic GasLab и GasRoom. Character V2 fixtures и shared production support KEEP.
- Единственный следующий cleanup-root удаляет только доказанный мусор и вводит короткое правило
  `legacy validator / no new consumers / delete after replacement proof`. Integration Lab, module
  functionality и debug framework в этом root не строятся.

## Прогресс общей сцены

Текущий этап: **M2 — audit завершён; безопасная очистка и no-growth boundary готовы к одному root**.

Следующий рекомендованный шаг: **один bounded V31 cleanup-root; после REPORT сразу вернуться к feature work**.
...
- [ ] **M2. Clean Integration Lab base** — сначала удалить четыре доказанно ненужных legacy families и
  заморозить рост семи временных validators; затем одна permanent playable scene принимает Dungeon
  Architect level profiles и реальные gameplay paths без generic Lab framework.
...
1. **Integration Lab:** один owner-present V31 cleanup-root удаляет четыре безопасных families и закрывает
   no-growth authority; саму Lab он не строит.
...
Текущее состояние: ownership утверждён; audit закончен, четыре DELETE-now families доказаны, семь
validators остаются frozen/no-growth; самой Lab scene ещё нет.

Осталось до M9:

- [x] Read-only классифицировать старые scene/support families как `DELETE / KEEP / BLOCKED` и назвать
  конфликтующие clauses product authority.
- [ ] Одним bounded cleanup-root удалить четыре доказанно ненужных families, запретить новые consumers
  retained validators и явно supersede-ить обещания нескольких permanent Labs.
...
- [ ] После cleanup REPORT перечитать current Gas SPEC/product и отдельно сформировать свежий V31 shape/PLAN.
...
- [ ] После cleanup REPORT сверить current SPEC/ADR и отдельно выбрать минимальный настоящий seam slice.
