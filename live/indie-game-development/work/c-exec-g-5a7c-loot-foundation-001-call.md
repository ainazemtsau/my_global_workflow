...
5. **Досягаемость.** Потолок подбора стоя ≈ `1.40` м; шкаф `2.10`, холодильник `2.00`, книжная полка
   `2.00` — выше него. Поведение на этот случай — **решение владельца**, и до его слова полки как
   опоры не авторятся: ставь десять предметов на пол, стол, стойку и тумбочку.
...
Привязка поведения к определению это чинит; подтверди числом в возврате.
