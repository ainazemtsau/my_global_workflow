...
- Frozen PLAN + В1 spec (authority — READ THIS FILE; absolute path on this machine, OS repo lives here):
  C:\my_global_workflow_worktrees\indie-game-development\live\indie-game-development\work\c-plan-characters-001-plan.md
  (do NOT invent the spec, do NOT build from this CALL alone — read that file).
...
- Worktree: ПЕРЕИСПОЛЬЗОВАТЬ существующий disposable спайк-worktree C:\projects\Unity\GasCoopGame_p2a0_002
  (спайк P2a0 принят/закрыт → он свободен; это НЕ dev/core). Завести ветку В1 от origin/main прямо в нём,
  переиспользовать уже импортированный Library и уже привязанный к нему редактор; git-гигиену билдер делает сам
  (сохранить свой MCP/package setup, убрать disposable Assets/_PuppetSpike). НЕ плодить новый worktree.
- НЕ трогать занятые GasCoopGame_dev / GasCoopGame_core (там foreign uncommitted работа).
