...
| Q5 *(ADDED c-converge-003 — the F1 emergent question)* | Does the COARSE band tier replicate to clients, and to WHAT consistency standard — given the latent conflict that the brief's exponential-relaxation Patankar coarse integrator is typically FLOAT while the locked core is integer-bit-exact (I4/C1)? | **DERIVED observable-first (CR1/CR2/CR3), no new owner fork.** (a) coarse REPLICATES (host-only DECIDED-OUT: crit-9 owner-approved on-wire ~11k coarse cells + the core read-gas mechanic + OR1/OR2 cross-peer consumer-independence); (b) standard = the locked DUAL guarantee generalized (lossless bit-exact oracle + lossy bounded-divergence-at-settle), the project's own consistency methodology, not a new decision; (c) the float/integer conflict is resolved by requiring the coarse AUTHORITATIVE/hashed/on-wire state be EXACT integer/fixed-point (I4/C1 extended) — float allowed only as a transient internal computation deterministically reduced before hashing. Carrier (locked-stream resolutionKey + SHALL-re-size vs separate plane), Q_coarse, divergence bound, interest scheme → PLAN. | the three silent PLAN escapes the verify named (host-only / float-hash / un-named bound) are each refuted by a signed fact or the LOCK; the standard did NOT require an owner fork (CALL condition unmet), so it is recorded as DERIVED with the refuted host-only reading named — mirroring how Q2's consistency obligation was DECIDED-not-forked. |
...
`§SIGNOFF: converge-arch ARCHITECT REPAIR — SIGNED-in-part @ 2026-06-16 (c-converge-003).` Q5 (the F1 emergent
question — coarse-tier replication + the float/integer conflict) worked observable-first and DERIVED from signed
properties + the LOCK (CR1/CR2/CR3) — no new owner fork; Q2's feedback half re-scoped to a NAMED deferral under
d-tempfeedback-001 (feedback post-g-d3a8) + §RESOLVED-4 (the owner-confirmed inter-layer read model). No pick
leaked into done_when — `arch_in_context_only: PASS` holds. The BINDING re-refutation is the next FRESH
converge-verify session (NOT self-certified here).
