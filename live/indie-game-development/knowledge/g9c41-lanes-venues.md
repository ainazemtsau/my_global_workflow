...
- Панель владельца: `work/board/dashboard.html` — РЕНДЕР поверх NOW+git (истина всегда NOW; расхождение = баг рендера). **Обязанность писаря: после каждого apply, меняющего open_calls/tasks/next, перегенерировать секции панели «📋 Борд», «Журнал» (LOG.md за последние 3 дня, сегодняшний день раскрыт `<details open>`, старше — с панели уходит) и «Проблемы» (открытые находки work/review/findings.md по приоритету)** (остальные секции — зафиксированные обсуждения, их правят только сессии, менявшие план). **Формат-правило владельца (2026-07-11, уточнено вечером): панель объясняется сама, и ей не жалко места.** Каждый ID (CALL/решение/слайс) на любой панели сопровождается пояснением человеческим языком. «Открытые работы» и «Ждёт тебя» рендерятся ПОЛНОШИРИННЫМИ карточками-строками (не узкими колонками): статус-плашка + человеческий заголовок + айди мелко, затем полными предложениями — «Что это / Зачем / Где сейчас / Что дальше» (для решений — «Что случилось / Вопрос / Рекомендация»; для закрытых — «Что было / Чем кончилось»). Дорога ставки — нумерованными шагами с расшифровкой каждого внутреннего имени. У борда есть постоянный «Словарь борда» (лег, dev/main, мерж-гейт, PLAN/BUILD, G5, чексум, ставка/аппетит/стоп-линия, спайк, канон/рамка/якорь) — описания не используют внутренний термин, которого нет в словаре: либо добавь статью, либо перефразируй. Владелец не должен помнить айдишники или лазить в документацию. **Правило чистоты панели: только актуальное** — журнал ≤3 дней, «Проблемы» только открытые, устаревшие секции-обсуждения при перегенерации ВЫРЕЗАЮТСЯ, а не копятся: каждая прошлая версия панели целиком лежит в git, ничего не теряется. То же правило — для канон-борда (canon repo board/dashboard.html).
