...
- Панель владельца: `work/board/dashboard.html` — РЕНДЕР поверх NOW+git (истина всегда NOW; расхождение = баг рендера). **Обязанность писаря: после каждого apply, меняющего open_calls/tasks/next, перегенерировать секцию «📋 Борд» панели** (остальные секции — зафиксированные обсуждения, их правят только сессии, менявшие план). Файл открывается в Chrome с диска; claude.ai-зеркало обновляет только Claude-сессия (опционально, не несущая часть).
