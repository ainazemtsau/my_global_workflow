...
direction_forecast:
  ...
  as_of: 2026-08-01
  basis: "Численная вероятность запрещена владельцем и не имеет эмпирического знаменателя. Ставки нет: направление между ставками, и прогнозировать не из чего. Концепт теперь есть и утверждён уставом, но ни одного узла под него ещё не построено."
  drivers:
    - "Концепт зафиксирован уставом 2026-07-31: команда налётчиков совместно выносит добычу из дома через ходы, которые она сама пробивает и закрывает. Неснимаемый минимум — перенос добычи плюс проход у́же груза, который делают сами игроки; тоннели как место заменяемы, как функция нет. Ядро описано, но ни одна его часть не проверена игрой."
    ...
    - "Концепт уточнён владельцем 2026-08-01 и стал ЗАМЕТНО определённее, но устав этого ещё не несёт — карта остановлена и маршрутизирована в frame (history/2026-08-01-s-map-burrow-concept-checkpoint-001.md). Хребет по его словам: тоннель — собственность, которую дорого сделать, приятно иметь и можно потерять, а хозяин — тот, кто её отбирает. Хозяин объявлен ЦЕНТРАЛЬНОЙ изюминкой: его поведение прописывается всерьёз, он замечает крыс, заливает, запенивает, разливает клей, загазовывает тоннели, среда на его стороне. Тоннели дорого копать, всё прорыть нельзя, хороший тоннель хочется защищать и не светить. Груз бывает разный, маршрут может идти через тоннель, по улице или подкопом, а способ переноски и число носильщиков НЕ фиксируются по прямому запрету владельца. ГЛОБАЛЬНОЙ СИМУЛЯЦИИ ГАЗА НЕТ: максимум — локальный короткий эффект в одной-двух норах, и он после демо. Сетевая модель выбрана его словами: считает хост, остальные показывают присланное, побитовое согласие независимых машин не требуется. Ничто из этого пока не является уставом."
    - "Вопрос «остаёмся в репозитории или новый» и вопрос «убрать старый код в архив» сверены 2026-08-01 тремя перепроверенными проходами. Первое ЗАПИСАНО (CHARTER.md:84, :120-128; слово владельца «остаёмся»), но его причина записана только в истории и перевёрнуто — как цена нового репозитория против срока 31 августа, а не как «здесь уже есть работающая обвязка»; первая формулировка истекает вместе со сроком. Второго НЕ СУЩЕСТВУЕТ НИГДЕ: настоящее указание владельца про архив от 2026-07-27 касалось концепт-фреймов, не кода. Устав переносу не препятствует («не удаляются» — про удаление, не про расположение), но и не разрешает. Цена переноса измерена: 188 отслеживаемых файлов вне Assets/GasCoopGame, и главная опасность — ложная зелень, три headless-проекта собираются успешно на пустой компиляции. Решение ноги по делегированию владельца: сейчас не переносим, новую игру пишем в новой папке с нуля, старый код держим закрытой уликой."
...
issues:
  - id: i-old-code-physical-move-deferred-001
    issue: "РЕШЕНИЕ ПРИНЯТО 2026-08-01 по явному делегированию владельца («с архивом кода посмотри, как лучше… прими решение, как лучше, чтобы мы как можно быстрее приступили к работе»): физический перенос старого игрового кода СЕЙЧАС не делается. Новая игра пишется в НОВОЙ папке с нуля; старый код остаётся на месте как закрытая улика — не читается, не цитируется, его тесты не чинятся. Записано, чтобы анализ не повторяли. ЦЕНА ПЕРЕНОСА: 188 отслеживаемых файлов вне Assets/GasCoopGame содержат этот путь (docs 83, openspec 58, tests 27, tools 13, validation.config, AGENTS.md, CLAUDE.md, три .csproj, CI). ГЛАВНАЯ ОПАСНОСТЬ — ЛОЖНАЯ ЗЕЛЕНЬ: core/, characters/ и render-contracts/ несут EnableDefaultCompileItems=false и единственный <Compile Include=\"..\\Assets\\GasCoopGame\\…\">, поэтому при неразрешимом пути dotnet build завершается успешно на ПУСТОЙ компиляции — воспроизведено первой рукой. Также: 27 тестовых файлов хардкодят игровые пути; сканы детерминизма падают закрыто и громко; hygiene.ps1 даёт один RED и один тихо исчезающий страж; семь токенов exists:Assets/GasCoopGame/… в ЗАМОРОЖЕННЫХ спеках станут неразрешимыми навсегда (validation.config:337 запрещает правку задним числом); .github/workflows/core-ci.yml идёт на push в main; пять worktree получат полный реимпорт Unity; перенос НЕВИДИМ для пакетных гейтов (delivering-scope.ps1:71 выводит область только из диффов openspec/docs). Unity-сторона дёшева: один ассет по GUID, все 19 asmdef по имени, .meta переезжают внутри Assets/. Обвязка игро-агностична примерно на 70%: измерительный слой (zero-float-scan:46, int-overflow-scan:56, type-hardcode-scan:53, mutation-scope, validation.config:284-322) хардкодит пути Core/Field/* и дублирован в четырёх местах — он переписывается под любой новый концепт независимо от переноса."
    level: execution
    route: work
    review_when: "В тот момент, когда под новую игру заводится первая папка с кодом и ей нужны гейты: перенос делается ОДНИМ движением вместе с их настройкой, потому что пять конфигураций сканов переписываются в любом случае. Обязательное условие той ноги: после переноса «нечего проверять» обязано давать КРАСНЫЙ, а не «успешно, 0 ошибок»; три .csproj правятся в том же движении. Отдельно нужно явное согласие владельца на то, что семь токенов в замороженных спеках станут неразрешимыми навсегда."
    evidence: "history/2026-08-01-s-map-burrow-concept-checkpoint-001.md; продуктовый репозиторий C:/projects/Unity/GasCoopGame (только чтение)."
  - id: i-product-constitution-unrouted-001
    ...
    level: execution
    route: work
    review_when: "Перед первой инженерной ногой нового концепта — раньше, чем будет написана первая строка новой игры. Иначе новая папка будет строиться под правила, написанные для игры, которую мы не делаем."
    evidence: "history/2026-07-31-s-frame-burrow-raiders-charter-revision-001.md (решение №2); history/2026-08-01-s-map-burrow-concept-checkpoint-001.md."
  - id: i-old-concept-material-in-live-001
    ...
    level: roadmap
    route: repair
    review_when: "Сразу после закрытия устава, отдельной ногой; список файлов показывается владельцу до перемещения. Канон-репозиторий в этот проход не входит — он уже лишён власти ограничением про Concept Lab."
    evidence: "history/2026-07-31-s-frame-burrow-raiders-charter-revision-001.md; history/2026-07-31-s-review-g-37a1-obsolete-concept-change-001.md; work/**; TREE.md g-37a1."
...
open_calls:
  - id: c-frame-burrow-concept-charter-deltas-001
    play: frame
    status: ready
    goal: "Привести устав в соответствие со словами владельца от 2026-08-01 пятью дельтами: игра проектируется на четверых (до восьми подключаются, спецсодержания нет); глобальной симуляции газа нет (максимум локальный эффект в одну-две норы, и он после демо); существующий код никогда не является доводом за игровое решение, новая игра пишется в новой папке с нуля, старый код — закрытая улика; концепт уточнён (хозяин центральная изюминка, тоннели собственность, которую дорого сделать и можно потерять, способ переноски НЕ фиксируется); остаёмся в репозитории из-за существующей обвязки."
    file: work/c-frame-burrow-concept-charter-deltas-001-call.md
    note: "Единственный планировочный фронтир при bet: null (G1). Наряд самодостаточен: несёт точный текст всех пяти дельт, дословные слова владельца под каждой, уже принятые и не переоткрываемые решения (сетевая модель — считает хост; перенос старого кода не делается; параллельные варианты переноски сняты; тоннель, а не пробой), вход в следующую за ним карточку первого узла и три отложенных вопроса по переноске. Владелец читает ТОЛЬКО чат и документов не открывает — весь точный текст показывается в чате целиком."
