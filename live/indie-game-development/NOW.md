...
decision_inbox:
  - id: d-arch-001
    note: |
      5 batched requirement clarifications from the architecture research
      (brief v2 §8, each with a recommendation): (a) R6 — does a 1 Hz cold rung
      still count as "never frozen"; (b) far-consequence latency sim <=100 ms /
      perceived 100-300 ms acceptable; (c) R7 — 2-3 shared bands + derived
      interface heights = "stratification represented"; (d) special gases
      day-one = scalar source/sink drivers, velocity-intent spike-gated;
      (e) perception ceiling — answer inside the render proof slice.
      Answer at or before c-shape-003. Source: history/s-arch-002.md.
