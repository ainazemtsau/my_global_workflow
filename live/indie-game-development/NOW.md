...
parallel_tracks:
  - id: g-7e15
    track: VISUAL / GASG
    state: active — Stage 2 DONE + merged/pushed to GasCoopGame main @c21c6bd (c-visual-006:
      gas passport + 96→128B layout ADR + W1b dominant-TypeId feed + real half-res + two-type
      preview; owner playtest passed, depth/clipping fix folded). No open visual CALL now; next
      visual step (Stage 3 hero one-gas polish / natural-jet / LP1 retest) is owner-gated planning,
      not auto-opened.
...
next:
  ...
  owner-gated. Parallel visual track: c-visual-006 closed/merged; no visual open_call.
