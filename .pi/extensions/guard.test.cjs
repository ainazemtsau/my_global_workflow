...
const CASES = [
  ...
  ["write", { path: "panel/build.py", content: "x" }, false, "запись в panel/"],
  ...
  ["write", { path: "panel/../os/KERNEL.md", content: "x" }, true, "обход через .."],
  ...
  ["bash", { command: "rm -rf panel" }, true, "rm -rf"],
  ["bash", { command: "echo hi > live/x.md" }, true, "редирект в live/"],
  ["bash", { command: "echo hi > os2/x.md" }, false, "редирект в os2/ разрешён"],
  ["bash", { command: "sed -i s/a/b/ panel/x.py" }, true, "sed -i"],
  ["bash", { command: "python panel/verify.py" }, false, "прогон проверки"],
