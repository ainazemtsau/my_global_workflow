...
| `finalize <engineering-call-id>` | **writer** | `os/adapters/coding-agent.md`, Role 1 finalize command: materialize the persisted engineering intent from fresh current state, run its phase guard, commit the finalized CALL, and stop. No collect, owner dialogue, or product work in the same job. |
