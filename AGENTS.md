...
| `MAINTENANCE REQUEST ...` (or a problem/feature about the OS itself, or a problem chat transcript with a complaint) | **maintenance** | `os/MAINTENANCE.md`. One problem per session. Never touch `live/**`; if the defect is in a direction-local play, diagnose/log it and hand back a ready direction RESULT/CALL patch path. |
...
3. **Keep G5 as a fresh refutation session; cross-family is optional unless explicitly requested.** Gate G5 ("done" needs evidence) is verified by *trying to refute* a claim in a SEPARATE session from the one that did the work. A different model family can be requested for extra rigor on a specific CALL, but it is not the default hard gate. Codex subagents are still same-session helpers, so they may provide a pre-pass only; the binding G5 pass is a fresh session. State in your RESULT whether evidence came from an in-session pre-pass or from the binding fresh review.
