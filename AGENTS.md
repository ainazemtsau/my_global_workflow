...
0. **Protocol lock comes before all Codex chatter.** The FIRST owner-facing line is the `📍 ...` header from `direction-os`; nothing precedes it. For an authorized day controller this is the plain `📍 День: ...` form: no route ids, play step, status codes or recited procedure. It still satisfies any skill announcement. During an active leg, app `final` is terminal RESULT/checkpoint only; otherwise keep working in commentary or ask the required owner question. A day controller starts another leg only on a later owner turn after a fresh Git read.
