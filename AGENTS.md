...
3. **Keep G5 as a fresh refutation session; model identity is never a gate.** Gate G5 ("done" needs evidence) is verified by *trying to refute* a claim in a SEPARATE physical chat from the one that did the work, including any day chat. A different model or provider may add optional diversity when available, but model identity or availability never affects review eligibility and cannot block the leg. Codex subagents are still same-session helpers, so they may provide a pre-pass only; the binding G5 pass is a fresh session. State in your RESULT whether evidence came from an in-session pre-pass or from the binding fresh review. A CALL phrase like "no heavy multi-agent refutation" reduces fan-out only; it does **not** waive G5, writer validation, or the requirement to leave the `open_call` open until the close evidence exists.
