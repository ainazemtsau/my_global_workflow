...
Workflow OS is a clean workflow kernel for making work state explicit, verifiable, and transportable across chats, tools, repositories, and future runtime surfaces.
...
This namespace is a new workflow. It does not inherit semantic authority from the legacy workflow architecture.
...
This Gate 1 install performs no migration. It creates only the shared kernel and protocol documentation under `workflow/**`.
...
Gate 2 execution specifications live under `workflow/execution/`.
...
Old workflow files are legacy evidence only. They are not design authority, runtime authority, or accepted state for the new workflow.

END_OF_FILE: workflow/core/00_WORKFLOW_OS.md
