...
Everything here is **read-only historical evidence**: previous workflow generations and their direction data. Nothing in this folder is authority for anything; no session edits it. The active system is `os/` (rules) + `live/` (direction state) at the repository root.
...
| `docs-legacy/` | Old device/tool setup notes (ChatGPT, Codex app, Trilium migration, new device) |

Why kept at all: outside-view material for reviews, and importable content (plans, knowledge). The full pre-v2 history also lives in branch/tag `legacy/vnext-r-main-before-proof-os-2026-05-25`.

END_OF_FILE: archive/README.md
