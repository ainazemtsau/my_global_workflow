...
status: ACCEPTED — OWNER VERDICT A
date: 2026-07-20
track: grid
node: g-4b92
source_call: c-work-grid-current-authority-audit-001
accepted: 2026-07-20 by s-work-grid-current-authority-audit-verdict-001

Этот документ — принятая владельцем фактическая и приоритетная основа следующего Grid planning leg.
Он не является engineering PLAN и ничего не разрешает реализовывать: BUILD остаётся закрыт до
отдельного принятого детального плана и законного product root.
...
Владелец сначала уточнил приоритет: `грид-модуль сейчас в приоритете` и `у нас горят сроки`. После
простого объяснения трёх маршрутов он выбрал рекомендацию точным ответом `A`.

Принято:

- factual audit и разделение Grid / Structure / Gas / Actor / Wind / Multiplayer ответственности;
- hybrid legacy-first: немедленный no-new-dependencies fence, затем минимальный Grid contract,
  replacement → proof → immediate deletion и permanent Integration Lab последней;
- Wind, generic flow framework, полный Multiplayer, damage/dose и неизвестные будущие слои не входят
  в критический путь Grid V1;
- следующий шаг — один owner-present детальный Grid V1 planning artifact, не BUILD.

Не принято и не разрешено этим вердиктом: product edits, физическое удаление legacy, запуск tests/
Unity, aperture repair, Wind/Multiplayer implementation или Integration Lab scene.
