...
## 9. Решение владельца перед M0: версия и исходная межмашинная база

status: OWNER-DECIDED 2026-07-18, вариант A; owner actual words: «принимаю A»

- Точная версия остаётся **Unity 6000.5.3f1**. Это ранее выбранная версия, а не новый выбор этого решения.
- Текущая продуктовая работа создаётся на Windows. Исходной authority для M0 считается не машина и не скопированная папка, а точное закоммиченное Git-состояние, которое свежий product registry признаёт текущей канонической базой при старте лега.
- Mac-миграция уже в основном выполнена и сейчас догоняет Windows-изменения. M0 не начинает её заново: существующая миграция сверяется и обновляется поверх актуальной Windows-произведённой Git-базы.
- M0 может закрыться только на одном exact reviewed commit, который воспроизводимо открывается и проходит требуемые проверки на обеих машинах. Передача целой project-папки, `Library` или локальных незакоммиченных изменений не является переносом authority.
- Старый candidate `df1e5ca` и прежние machine-local пути остаются историческим evidence/input, но не launch authority. Точные branch/path/SHA определяет свежая product authority в самом M0-леге.
- Это решение фиксирует источник и способ схождения; оно не объявляет Unity migration формально доставленной и не запускает product BUILD.
