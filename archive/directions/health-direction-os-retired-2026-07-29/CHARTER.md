...
constraints:
  ...
  - |
    Системная архитектура Health AI System (принято 2026-06-14, core-first re-frame):
    - Провайдеро-независимость: система — это plain Markdown/YAML файлы в git, которыми
      управляет любой LLM (минимум ChatGPT, Claude, Claude Code, Codex). Конвенции живут в
      AGENTS.md + CLAUDE.md (ссылается на AGENTS.md) + переносимом system-prompt для вставки
      в project instructions (ChatGPT/Claude Projects). Без зависимости от проприетарной
      памяти/инструментов/runtime провайдера; числа хранятся в файлах, арифметика — в
      задокументированных формулах, чтобы разные провайдеры давали один ответ.
    - Интерфейс владельца — обычный текст/голос/фото; система парсит во входной черновик и
      задаёт уточняющий вопрос только при существенной двусмысленности. Принцип
      research-and-decide: система решает экспертные переменные, спрашивает только
      неустранимые личные факты.
    - Само-расширяемость: новые глобальные процедуры/функции добавляются через определённый
      workflow; новый домен = 1 папка + 1 строка реестра — без переписывания ядра.
    - Сильное ядро (g-health-core) строится первым; питание и тренировки — модули на ядре
      с общими метриками/фазами и кросс-модульным обменом данными.
  - |
    product_repos:
