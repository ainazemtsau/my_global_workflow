...
old_source_path: workflow_v3/runbooks/DIRECTION_PROJECT_ROOT_BOOTSTRAP_RUNBOOK.md

old_file_retained_as_migration_source: true

## purpose

Represent the registered setup-only root bootstrap entrypoint without executing the old source body in the active procedure model.

## trigger_when_to_use

- Use when START selects `direction_project_root_bootstrap`.

## when_not_to_use

- Do not use to define Direction semantics, create runtime state, or perform storage work.
...
- requested Direction setup context.
...
purpose: prevent execution of unmigrated setup logic
