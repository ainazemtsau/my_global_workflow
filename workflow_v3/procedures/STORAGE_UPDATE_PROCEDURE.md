...
old_source_path: workflow_v3/control_plane/STORAGE_UPDATE_PROTOCOL.md

old_file_retained_as_migration_source: true

## purpose

Represent the registered storage update adapter entrypoint without executing the old source body in the active procedure model.

## trigger_when_to_use

- Use when START selects `persist_accepted_state`.

## when_not_to_use

- Do not use to mutate repository or runtime state until the body is migrated.
...
- admitted storage package context.
...
purpose: prevent execution of unmigrated storage mutation logic
