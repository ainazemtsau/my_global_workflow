...
[ ] START_CONTRACT included start_goal, terminal_condition, child_call_policy, completion contract, declared stages, boundaries, and sources.
[ ] RUN represented declared material stages with STAGE_RESULT and no runtime stage compression.
[ ] Any child/adaptor call used CHILD_PROCEDURE_CALL, entered RUN_WAITING_FOR_CHILD_RETURN, and required verified CHILD_PROCEDURE_RETURN before reliance.
[ ] First chat emitted CLOSURE_CHECK against the selected completion contract.
[ ] CLOSURE_CHECK and FINISH blocked on open, missing, or unverified child returns and missing validation/evidence.
...
[ ] Continuation copy-paste content is complete when NEXT_CHAT_CARD is needed and does not carry unfinished child work.
[ ] Code-assistant child/adaptor transfer test requires complete CHILD_PROCEDURE_CALL or lifecycle-position-appropriate transfer packet.
...
code_assistant_child_call_transfer_test_result:
