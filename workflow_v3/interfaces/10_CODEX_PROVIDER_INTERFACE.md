...
This interface summarizes the code-assistant/provider boundary for Workflow v3. Code/repository mutation is a `code_repository_dependency` and runs only through Codex/code assistant. Non-mutating support dependencies are separate dependency surfaces.
...
The parent enters `RUN_WAITING_FOR_DEPENDENCY_RETURN`; the return must come back as `DEPENDENCY_RETURN` / `CODEX_RETURN_PACKET`, match the call, resume the same selected main procedure, and pass `DEPENDENCY_RETURN_VERIFICATION` before reliance. Prior packet labels are unsupported.
...
Non-mutating check, research, analysis, evidence, and real human decision support use `support_dependency`. GitHub or file access from ChatGPT parent is read or verification only unless a separate admitted write executor returns verifiable write evidence.
