...
- Closure derives continuation from selected main procedure, actual result, passed or explicitly blocked completion check, resolved child-call status, and next intended workflow step.
...
- `NEXT_CHAT_CARD` is used only after current START goal completion or explicit blocked closure and never carries unfinished child work.
...
- Card represents unfinished child work, replaces `CHILD_PROCEDURE_CALL`, or closes while child returns are open/missing/unverified.
