...
*   `DONE`: all requested work within scope is complete, required validators and read-back are satisfied, evidence is present, and forbidden targets were not touched.
...
*   `FAILED`: Codex attempted work and a validator, write, read-back, or safety check failed.
...
4.  Required read-back was performed and passed.
...
## Required Trilium install return compatibility

When the wave is specifically a Trilium install operation, Codex must use the more specific `CODEX TRILIUM INSTALL RESULT` format required by the universal installer.
...
*   A write claim without read-back when read-back is required.
...
*   `Required read-back was performed and passed`
