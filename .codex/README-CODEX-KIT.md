...
| AGENTS.md loaded (not truncated) | start a session here; ask: *"Quote the first hard rule from this repo's AGENTS.md and the title of the Codex section."* | It quotes "One atomic leg = one job…" **and** "Codex: how to work this OS". If the section is missing, AGENTS.md likely exceeded the byte cap → trim it. |
...
| Explicit fan-out works | *"Spawn three read-only children in parallel on `<a tiny prompt>`, wait for all, summarize each."* | Three child threads run and report back (native Ultra may delegate proactively elsewhere; this repo restricts Direction-OS legs to play-triggered fan-out). |
